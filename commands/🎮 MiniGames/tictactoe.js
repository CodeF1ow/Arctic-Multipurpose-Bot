...
class TicTacToe {
        ...
        constructor(options) {
                ...
                if (!options.opponent) throw new TypeError('Argumento faltante: oponente | Tipo: DiscordUser')
                if (!options.message) throw new TypeError('Falta el argumento: mensaje | Tipo: Message')
        ...
        async start() {
                ...
                if (midDuel.has(author)) {
                        return this.message.reply(`Actualmente estás en un duelo`)
                } else if (midDuel.has(member.id)) {
                        return this.message.reply(`<@${member.id}> está actualmente en un duelo`)
                }
                if (member.id === this.message.client.user.id) {
                        return this.message.reply("No puedes batirte en duelo conmigo.")
                }
                ...
                if (player == 0) {
                        Embed = new MessageEmbed()
                                ...
                                .setDescription(`Es ${authorName}tu Turno!`)
                                .setColor(3426654)
                } else {
                        Embed = new MessageEmbed()
                                .setTitle(`🎮 ${authorName} VS __**${this.opponent.username}**__ 🎮`)
                                .setDescription(`Es ${this.opponent.username}tu Turno!`)
                                .setColor(3426654)
                }
                        ...
                        gameCollector.on('collect', async btn => {
                                if (btn.customId == a11 && gameData[player].member.id === btn.user.id) {
                                        ...
                                        if (btn.label == this.oEmoji || btn.label == this.xEmoji) { // User tries to place at an already claimed spot
                                                btn.message.update('Ese lugar ya está ocupado.')
                                        } else {
                                                try {
                                                        ...
                                                        if (
                                                                ...
                                                                this.message.reply(`Es un **empate**!`)
                                                                gameCollector.stop()
                                                                midDuel.delete(author)
                                                                midDuel.delete(member.id)
...
module.exports = {
        ...
        aliases: ["ttt"],
        category: "🎮 Minijuegos",
        description: "Permite jugar a un juego de tres en raya",
        usage: "tictactoe --> Juega el juego",
        type: "buttons",
        run: async (client, message, args, cmduser, text, prefix) => {
                ...
                if (!opponent) return message.reply(`:x: **Por favor, menciona a quién quieres retar en tictactoe.**`);
                new TicTacToe({

...
module.exports = {
    ...
    description: "Los comandos de voz están ahí para los COMANDOS DE UNIÓN A LA CREACIÓN, utilícelos para ajustar su canal alojado!",
      ...
      if (cmd === "lock") {
        ...
        if (client.jointocreatemap.get(`tempvoicechannel_${message.guild.id}_${channel.id}`)) {
          ...
          if (!owner)
            ...
            if(!vc.permissionsFor(vc.guild.me).has(Permissions.FLAGS.MANAGE_CHANNELS)){
              return message.reply(`:x: **Me falta el MANAGE_CHANNEL PERMISSION para \`${vc.name}\`**`)
            }
        ...
        if (client.jointocreatemap.get(`tempvoicechannel_${message.guild.id}_${channel.id}`)) {
          ...
          if (!owner)
            ...
            .then(lol => {
              ...
              return message.reply({embeds : [new Discord.MessageEmbed()
                .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                .setTitle("✅ ESTABLEZCA su canal!")
                .setDescription(`Ya nadie puede hablar!`)
                .setFooter(client.getFooter(es))
              ]})
            })
        ...
        if (client.jointocreatemap.get(`tempvoicechannel_${message.guild.id}_${channel.id}`)) {
          ...
          vc.permissionOverwrites.edit(message.guild.id, {
            ...
            return message.reply({embeds : [new Discord.MessageEmbed()
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
              .setTitle("✅ Destape su canal!")
              .setDescription(`Ahora todo el mundo puede hablar en su Canal!`)
              .setFooter(client.getFooter(es))
            ]})
          })
        } else {
        ...
        if (client.jointocreatemap.get(`tempvoicechannel_${message.guild.id}_${channel.id}`)) {
          ...
          try {
            ...
            if(!channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.CREATE_INSTANT_INVITE)){
              return message.reply(`:x: **Me falta el CREATE_INSTANT_INVITE PERMISSION para \`${channel.name}\`**`)
            }
            channel.createInvite().then(invite => {
              ...
              vc.permissionOverwrites.edit(member.user.id, {
                ...
                member.user.send({embeds : [new Discord.MessageEmbed()
                  ...
                  .setDescription(`[Haga clic aquí](${invite.url}) para unirse **${channel.name}**\n\n${txt ? txt : ""}`.substring(0, 2000))
                  .setFooter(client.getFooter(es))
                ]}).catch(e => {
                  return message.reply({embeds : [new Discord.MessageEmbed()
                    .setColor(es.wrongcolor)

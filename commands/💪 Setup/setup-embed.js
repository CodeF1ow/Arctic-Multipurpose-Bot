...
module.exports = {
  ...
  description: "Cambie el aspecto de sus incrustaciones (color, imagen, miniatura, ...)",
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
        ...
        collector.on('collect', async button => {
          if (button?.user.id === cmduser.id) {
            ...
            if (button?.customId == "1") {
              ...
              tempmsg = await tempmsg.edit({embeds: [new Discord.MessageEmbed()
                ...
                .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                .addField(`**Colores admitidos:**`, `>>> **HTML/Hex-Colors** (\`#ffff00\`)\n\nNombres de los colores/colores de los Discord (\`FUCHSIA\`)\n\nColores de notación hexadecimal (\`0xffffff\`)`)
                .addField(`**Colores compatibles con Discord:**`, `>>> ${discordsupportedcolors.map(c => `\`${c}\``).join("︲")}`)
                .addField(`**Color actual:**`, `>>> \`${es.color}\``)
                ...
                .then(collected => {
                  ...
                  try {
                    ...
                    try {
                      ...
                      if(!newcolor) throw { message: "Color inválido añadido, asegúrese de atenerse a las reglas de ejemplo" };
                    }catch (e){
                      return message.reply({embeds: [new MessageEmbed().setColor("RED").setTitle(":x: COLOR NO VÁLIDO AÑADIDO").setDescription(`\`\`\`${String(e.message ? e.message : e).substring(0, 2000)}\`\`\``)]})
                    }
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                ...
                .then(collected => {
                  ...
                  try {
                    ...
                    es = client.settings.get(message.guild.id, "embed")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`​✔️ El nuevo texto de pie de página para incrustar es:`.substring(0, 256))
                      .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          else button?.reply({content: `❌​ No está permitido hacer eso! Sólo: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `​✔️ **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });

 ...
 module.exports = {
   ...
   description: `Añade un Rol a cada HUMANO en este servidor`,
   type: "memberrole",
   run: async (client, message, args, cmduser, text, prefix) => {
     ...
     try {
      ...
      return message.reply({embeds : [new MessageEmbed()
       ...
       var members = message.guild.members.cache.filter(member => !member.roles.cache.has(role.id) && !member.user.bot).map(this_Code_is_by_Kiri86_8565 => this_Code_is_by_Kiri86_8565);
       ...
       function send_finished() {
         ...
         message.reply({
           ...
           embeds: [new MessageEmbed()
             ...
             .setDescription(`Añadido con éxito ${role} a \`${success} HUMANOS\` de \`${counter} HUMANOS\`${failed != 0 ? `\n${failed} Miembros, no consiguieron el añadido, rehacerlo con: \`${prefix}addroletohumans ${role.id}\``: ""}`)
          ]});
       }

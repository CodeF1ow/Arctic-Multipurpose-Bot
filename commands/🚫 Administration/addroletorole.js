 ...
 module.exports = {
   ...
   description: `Añade un Rol a cada MIEMBRO que tenga un Rol en este Servidor`,
   type: "memberrole",
   run: async (client, message, args, cmduser, text, prefix) => {
     ...
     try {
      ...
      if (role.id == rolehas.id)
       ...
       var members = message.guild.members.cache.filter(member => member.roles.cache.has(rolehas.id) && !member.roles.cache.has(role.id)).map(this_Code_is_by_Kiri86_8565 => this_Code_is_by_Kiri86_8565);
       ...
       function send_finished() {
         ...
         message.reply({
           ...
           embeds: [new MessageEmbed()
             ...
             .setDescription(`Añadido con éxito ${role} a \`${success} Miembros\` de \`${counter} Miembros\`${failed != 0 ? `\n${failed} Miembros, no consiguieron el añadido, rehacerlo con: \`${prefix}addroletorole ${rolehas.id} ${role.id}\``: ""}`)
          ]});
       }

...
module.exports = {
  ...
  description: "Permite DM a un USUARIO o a todos los USUARIOS de un ROL",
  type: "memberrole",
      ...
      if(member){
        ...
        try{
          member.send({embeds : [new MessageEmbed()
            ...
            .setFooter(client.getFooter(es))
            .setAuthor(`Mensaje de: ${message.author.username}`, message.author.displayAvatarURL({dynamic:true}), "https://arcticbot.xyz/discord")
        ...
        var members = message.guild.members.cache.filter(member=> member.roles.cache.has(role.id) && !member.user.bot).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);  
        var failed = [];
        var succeeded = [];
        message.delete().catch(e => console.log("No se ha podido borrar el mensaje, esto es una trampa para evitar la caída."))
        ...
        for(const member of members) {
          try{
            ...
            await member.send({embeds : [new MessageEmbed()
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
              .setFooter(client.getFooter(es))
              .setAuthor(`Message from: ${message.author.username}`, message.author.displayAvatarURL({dynamic:true}), "https://arcticbot.xyz/discord")
              .setDescription(args.slice(1).join(" ").substring(0, 2048))
            ]}).catch(e=>{
              failedd = true
            })

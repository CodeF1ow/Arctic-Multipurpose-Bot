...
module.exports = {
  ...
  cooldown: 2,
  usage: "instantclose",
  description: "El instante cierra el billete",
  type: "channel",
      ...
      message.reply({
          ...
          collector.on('collect', async b => {
              if (b?.user.id !== buttonuser.id)
                  return b?.reply(`❌​ **Sólo el que escribió ${prefix}help se le permite reaccionar!**`, true)
                ...
                msg.edit({
                    ...
                    if(ticketCh && ticketCh.type == "GUILD_CATEGORY") {
                        if(ticketCh.children.size < 50) {
                            await msg.channel.setParent(ticketCh.id, { lockPermissions: false }).catch(async (e) => {
                                await msg.channel.send(`No se puede trasladar a: ${ticketCh.name} (\`${ticketCh.id}\`) porque se ha producido un error:\n> \`\`\`${String(e.message ? e.message : e).substring(0, 100)}\`\`\``).catch(() => {});
                            })
                        } else {
                            await msg.channel.send(`Categoría de entradas ${ticketCh.name} (\`${ticketCh.id}\`) está lleno, no puede moverse!`).catch(() => {});
                        }
                    } else {
                        await msg.channel.send(`No se pudo encontrar ${closedParent} como padre`).catch(() => {});
                    }
                  } 
                  ...
                  msg.channel.send({
                      ...
                      embeds: [new Discord.MessageEmbed()
                          ...
                          .setDescription(`Cerrado el billete de <@${data.user}> y lo sacó del Canal!`.substring(0, 2000))

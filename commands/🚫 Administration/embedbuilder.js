...
module.exports = {
  ...
  aliases: ["embedb"],
  cooldown: 2,
  usage: "embedbuilder --> seguir los pasos",
  description: "Reenvía un mensaje de u como un Embed\n\n Para tener, por ejemplo, ningún título hacer que:  embed ++ Este es el aspecto de un Embed sin imagen",
  type: "server",
...
let footerImage = new MessageButton()
    .setLabel("Imagen de pie de página")
    .setStyle('PRIMARY')
...
let buttonEvent = async (interaction) => {
  ...
  if (interaction?.message.id === msg.id) {
    if (interaction?.user.id === message.member.id) {
      ...
      await interaction?.reply({content : `❌​ No está permitido hacer eso! Sólo: <@${message.author.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
...
let embedEditing = async(button) => {
    ...
    if(id.startsWith(`builder`)) {
        ...
        button?.message.edit({content : `<a:Loading:833101350623117342> **Por favor, envíeme su opinión ahora!**`,
    ...
    if(id == `cancel`) {
        button?.message.edit({content :`Cancelación de...` ,components:null}) 

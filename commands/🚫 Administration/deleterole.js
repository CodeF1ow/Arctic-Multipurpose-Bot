...
module.exports = {
  ...
  description: `Borra un rol de este servidor`,
  type: "role",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let button_verify = new MessageButton().setStyle('SUCCESS').setCustomId('deleterole_verify').setLabel("Verifique este paso").setEmoji("833101995723194437")
      ...
      collector.on('collect', async b => {
          ...
          if (b?.customId == "deleterole_verify") {
            ...
            role.delete(`${message.author.tag} Solicita una eliminacion de rol`)
              .then(r => {
      ...
      collector.on('end', collected => {
          if (!edited) {
              ...
              msg.edit({
                  content: `<@${message.author.id}>`,
                  embeds: [endedembed],
                  components: [new MessageActionRow().addComponents(button_verify.setDisabled(true).setLabel("NO SE HA PODIDO VERIFICAR").setEmoji("833101993668771842").setStyle('DANGER'))]
              }).catch((e) => {
                  console.log(String(e).grey)
              });
          }

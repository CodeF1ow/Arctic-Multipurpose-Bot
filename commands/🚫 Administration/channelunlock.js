...
module.exports = {
  ...
  cooldown: 2,
  usage: "channelunlock",
  description: "Desbloquea un Canal de Texto al instante",
  type: "channel",
        ...
        if(channel.isThread())
          ...
          if(channel.permissionOverwrites.cache.filter(permission => permission.deny.toArray().includes("SEND_MESSAGES")).size < 1)
            return message.reply({embeds :[new MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(client.getFooter(es))
              .setTitle(`❌​ **Este canal no está bloqueado!**`)
              .setDescription(`Esto suele significar, que los **permisos** del canal están definidos de tal manera, que __todos__ permiten enviar un mensaje!`)
            ]});
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`​✔️ **Desbloqueado con éxito \`${channel.name}\`**`)
          ]});

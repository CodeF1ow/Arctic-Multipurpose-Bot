...
module.exports = {
    ...
    description: `Aprueba, rechaza o incluso crea una sugerencia de su SETUP!`,
        ...
        try {
          ...
          if (([...message.member.roles.cache.values()] && !message.member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(message.author.id) && ([...message.member.roles.cache.values()] && !message.member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(message.guild.ownerId, config.ownerid).includes(message.author.id) && !message.member.permissions.has([Permissions.FLAGS.ADMINISTRATOR]))
            ...
            client.settings.ensure(message.guild.id, {
              suggest: {
                channel: "",
                approvemsg: `​✔️ Sugerencia aceptada! Pronto estara lista.`,
                denymsg: `❌​ Gracias por los comentarios, pero no estamos interesados en esta Sugerencia en este momento.`,
                maybemsg: `💡 Estamos pensando en esta sugerencia!`,
                duplicatemsg: `💢 Esta es una sugerencia duplicada`,
                soonmsg: `👌 Espere esta sugerencia pronto!`,
                statustext: `<a:Loading:833101350623117342> A la espera de los comentarios de la comunidad, por favor vote!`,
            ...
            const embed = new MessageEmbed()
                ...
                .setColor(color)
                .setFooter(client.getFooter(`Quieres sugerir algo? Simplemente escríbelo en este canal`, "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/light-bulb_1f4a1.png"))
              
            if(embed.fields[2]){
              embed.fields[2].name == `➡️​ __Razón por **${message.author.tag}**:__`;
              embed.fields[2].value == `>>> ${String(reason).substring(0, 1000)}`;
            } else {
              embed.addField(`➡️​ __Razón por **${message.author.tag}**__`, `>>> ${String(reason).substring(0, 1000)}`)
            }
            ...
            try{
              ...
              if(member){
                member.send({content: `Su sugerencia en **${message.guild.name}** obtuvo una actualización de estado!\n> https://discord.com/channels/${message.guild.id}/${channel.id}/${targetMessage.id}`,embeds: [embed]})
              }

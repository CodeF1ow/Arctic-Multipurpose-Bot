  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try {
      ...
      .setAuthor(client.getAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
        dynamic: true
      }), "https://discord.com/api/oauth2/authorize?client_id=1015480877658157136&permissions=8&scope=applications.commands%20bot"))
      ...
      embed.addField("**➡️​ Canal de Reglas:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: "❌​ \`Sin canal\`"}`, true)
      embed.addField("**➡️​ Canal público de actualizaciones:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: "❌​ \`Sin canal\`"}`, true)
      embed.addField("**➡️​ Canal AFK:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: "❌​ \`No hay canal\`"}`, true)

      embed.addField("**➡️​ Nivel de verificación:**", `\`${message.guild.verificationLevel}\``, true)
      embed.addField("**➡️​ Filtro de contenidos explícitos:**", `\`${message.guild.explicitContentFilter}\``, true)
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "<:nitro_logo:966470998557880351> \`" + message.guild.premiumSubscriptionCount + "\`", true)

      embed.addField(client.la[ls].cmds.info.serverinfo.field13, `<:nitro_logo:966470998557880351> \`${boostlevel}\``, true)
      embed.addField(client.la[ls].cmds.info.serverinfo.field14, "👾 \`" + maxbitrate + " kbps\`", true)
      if(boosts >= 14){
          embed.addField(`**➡️​ Vanity:**`, `${message.guild.vanityURLCode ? `https://discord.gg/${message.guild.vanityURLCode}` : "❌​ Sin Vanidad-Invitación"}`)
      }
      ...
      if(message.guild.banner) {
        ...
        .setDescription(`[Enlace de descarga](${message.guild.bannerURL({size: 1024})})${message.guild.discoverySplash ? ` | [Enlace de la imagen](${message.guild.discoverySplashURL({size: 4096})})`: ""}\n> This is the Image which is shown on the Top left Corner of this Server, where you see the Channels!`)
        ...
        .setDescription(`[Enlace de descarga](${message.guild.discoverySplashURL({size: 1024})})${message.guild.banner ? ` | [Enlace de la imagen](${message.guild.bannerURL({size: 4096})})`: ""}\nThis is the Image you see when you get invited to this Server on the official Discord Website!`)

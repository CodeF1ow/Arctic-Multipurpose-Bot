...
module.exports = {
  ...
  aliases: ["ubanner"],
  category: "🔰 Info",
  description: "Obtener el Banner de un usuario",
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {   
      ...
      //create the EMBED
      const embeduserinfo = new MessageEmbed().setColor(es.color).setTitle(`Banner de ${user.tag}`)
      let banner = false;
      try{
      ...
      if(banner) {
        ...
        embeduserinfo.setDescription(`[Descargar el banner de **${user.tag}**](${banner}) <@${user.id}>`)
      } else {
        embeduserinfo.setColor(user.hexAccentColor ? user.hexAccentColor : es.color)
        embeduserinfo.setTitle(`❌ **No tiene Banner!**`)
      }

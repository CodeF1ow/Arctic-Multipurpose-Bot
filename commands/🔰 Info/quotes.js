...
module.exports = {
  ...
  aliases: ["quos", "quote"],
  category: "🔰 Info",
  description: "Muestra las cuotas que se guardan en este usuario/tú",
  usage: "quotes [@USER]",
  type: "user",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(!data || data.length == 0) return message.reply({content: ":x: **Este usuario aún no tiene citas en este servidor!**"})
      ...
      swap_pages(client, message, datas, `Cuotas de **\`${user.tag}\`** en **\`${message.guild.name}\`**\n*(Clasificado por fecha)*\n Para más detalles, escriba:\n> \`${prefix}quotes ${user.id} [ID]\``);
    } catch (e) {

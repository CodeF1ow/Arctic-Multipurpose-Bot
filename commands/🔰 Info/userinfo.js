...
const flags = {
	...
	BUGHUNTER_LEVEL_1: 'Cazador de bug (Level 1)',
	BUGHUNTER_LEVEL_2: 'Cazador de bug (Level 2)',
    ...
    try {   
      ...
      try{
        ...
        const activity = member.presence ? member.presence.activities[0] : {
          ...
          state : "OFFLINE - Sin actividad"
        };
        //create the EMBED
        const embeduserinfo = new MessageEmbed()
        embeduserinfo.setThumbnail(customavatar ? customavatar : member.user.displayAvatarURL({ dynamic: true, size: 512 }))
        embeduserinfo.setAuthor(handlemsg(client.la[ls].cmds.info.userinfo.author, { usertag: member.user.tag}), member.user.displayAvatarURL({ dynamic: true }), "https://arcticbot.xyz/discord")

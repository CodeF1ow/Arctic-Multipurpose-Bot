...
module.exports = {
  name: `savedqueue`,
  category: `⚜️ Cola(s) personalizada(s)`,
  aliases: [`savequeue`, `customqueue`, `savedqueue`],
  description: `Guarda la cola actual en un nombre`,
...
**Opciones**:
> \`elija la pista que desea eliminar\``,
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      case `addcurrenttrack`: {
        ...
        //return susccess message
        return message.reply({embeds : [new MessageEmbed()
          .setTitle(`​✔️ Añadido ${track.title} en la cola \`${Name}\``.substring(0, 256))
      ...
      case `removesong`: {
        ...
        //return susccess message
        return message.reply({embeds :[new MessageEmbed()
          .setTitle(`​✔️ Borrado ${deletetrack.title} de la cola \`${Name}\``.substring(0, 256))
      ...
      case `mix`: {
        ...
        //return susccess message
        return message.reply({embeds : [new MessageEmbed()
          .setTitle(`​✔️ Barajado ${newtracks.length} Pistas en cola \`${Name}\``.substring(0, 256))
      ...
      case `removeduplicates`: {
        ...
        //return susccess message
        return message.reply({embeds : [new MessageEmbed()
          .setTitle(`​✔️ Eliminado ${counter} Pistas de la cola \`${Name}\``.substring(0, 256))
      ...
      case `list`: {
        ...
        for (const item in queues) {
          ...
          description += `**❯ ${item}** | \`${queues[item].length} Pistas\`\n`
        }
        //return susccess message
        return swap_pages(client, message, description, `Sus colas guardadas`)
      }
      break;
      case `createsave`:
      case `cs`:
      ...
      case `details`: {
        ...
        return swap_pages(client, message, array, `Información detallada sobre: \`${Name}\` [${tracks.length} Pistas]`)
      }
      break;
      default:
        return message.reply({embeds : [new MessageEmbed()

...
module.exports = {
	...
	category: "⌨️ Programacion", //the category this will be listed at, for the help cmd
	...
	usage: "npm <package>", //this is for the help command for EACH cmd
  	description: "Buscar en el Registro NPM información sobre un paquete", //the description of the command
  	...
  	run: async (client, message, args, cmduser, text, prefix) => {
		...
		try {
			...
			const body = await fetch(`https://registry.npmjs.com/${pkg}`)
				.then((res) => {
				if(res.status === 404) throw "No se han encontrado resultados.";
				return res.json();
				});
			...
			return message.reply({ embeds: [new MessageEmbed()
				...
				.setDescription([
				body.description || "No hay descripción.",

    ...
    try {
      ...
      ensure_economy_user(client, message.guild.id, user.id)
        ...
        for(let i = 0; i < sorted.length; i++){
          ...
          stringarray.push(`**${theindex}. \`${tuser.tag}\`** ・ ${tuser}\`\`\`yml\nBolsillo: ${nFormatter(Math.floor(data.balance))} 💸 ・ Banco: ${nFormatter(data.bank)} 💸 ・ [${items}] Items: ${nFormatter(itemsvalue)} 💸\`\`\``)
        }
        
        
        const description = stringarray;
        const TITLE = `${message.guild.name} | Tabla de clasificación económica 💸`
        ...
        if(embeds.length == 0) return message.reply(":x: Todavía no hay tabla de clasificación!**")
        if (embeds.length === 1) return message.reply({embeds: [embeds[0]]}).catch(e => console.log("ESTO ES PARA EVITAR UN CHOQUE"))
        const queueEmbed = await message.reply(
          {embeds: [embeds[currentPage].setFooter(client.getFooter(`Lo eres: Rango #${yourrank} ・ Página: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true})))]}
        ).catch(e => console.log("ESTO ES PARA EVITAR UN CHOQUE"))
        let reactionemojis = ["⏪", "⏩"];
        try {
          ...
          try {
            if (reaction.emoji?.name === reactionemojis[1] || reaction.emoji?.id  === reactionemojis[1]) {
              if (currentPage < embeds.length - 1) {
                currentPage++;
                queueEmbed.edit({embeds: [embeds[currentPage].setFooter(client.getFooter(`Tú eres: Rango #${yourrank} ・ Página: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true})))]});
              } else {
                currentPage = 0
                queueEmbed.edit({ embeds: [embeds[currentPage].setFooter(client.getFooter(`Tú eres: Rango #${yourrank} ・ Página: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true})))]});
              }

...
module.exports = client => {
    ...
    async function check(client){
        console.log(" [YOUTUBE] :: Comprobando cuentas...".italic.brightRed)
            ...
            try{
                ...
                for(const yt of client.social_log.get(guildid, "youtube.channels")){
                    client.youtube_log.ensure(yt, {
                        oldvid: "",
                        alrsent: [],
                        message: "**{videoAuthorName}** Nuevo video subido \`{videoTitle}\`!\n**Mira:** {videoURL}"
                    })
                }
                client.social_log.get(guildid, "youtube.channels").forEach(async (youtuber) => {
                    console.log(` [YOUTUBE] :: [${youtuber}] | Empezar a comprobar...`.italic.brightRed);
                    let channelInfos = await channelInfo(youtuber);
                    if(!channelInfos) return console.log(String(" [YOUTUBE] :: [ERROR] | Youtuber inválido proporcionado: " + youtuber).italic.brightRed);
                    //channelInfos.url, ChannelDATA
                    let video = await checkVideos(channelInfos.url, youtuber);
                    if(!video) return console.log(` [YOUTUBE] :: [${channelInfos.name}] | No hay notificación`.italic.brightRed);
                    
                    let channel = await client.channels.fetch(client.social_log.get(guildid, "youtube.dc_channel")).catch(e=>{return;})
                    if(!channel) return console.log(" [YOUTUBE] :: [ERROR] | Canal Discord no encontrado".italic.brightRed);
                    ...
                    console.log(" [YOUTUBE] :: Notificación enviada !".italic.brightRed);
                });        
            }catch (e){
                console.log(String(" [YOUTUBE] :: " + e).grey)
            }
        })
    }

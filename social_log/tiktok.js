...
module.exports = client => {
    ...
    async function check(guild){
        return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} :: Actualmente está deshabilitado el registrador TIKTOK`.italic.brightMagenta)
        console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Comprobando cuentas...`.italic.brightMagenta)
        ...
        try {
            ...
            if(!client.social_log.get(guild.id, `tiktok.channels`) || client.social_log.get(guild.id, `tiktok.channels`).length < 1) {
                return  console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Ya no tiene canales de TikTok!`.italic.brightMagenta);
            }
            for(const tt of client.social_log.get(guild.id, `tiktok.channels`)){
                client.tiktok.ensure(tt, {
                    oldvid: ``,
                    message: `**{videoAuthorName}** subido \`{videoTitle}\`!\n**Mira:** {videoURL}`
                })
            }
            ...
            if(!channel) return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: | No se ha encontrado ningún canal de discord`.italic.brightMagenta);
           
            client.social_log.get(guild.id, `tiktok.channels`).forEach(async (tiktoker) => {
            console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: [${tiktoker}] | Empezar a comprobar...`.italic.brightMagenta);
                try {
                    ...
                    if(!posts.collector[0]) return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} [${tiktoker}] :: NO SE ENCUENTRA / No hay puestos!`.italic.brightMagenta) 
                    ...
                    if(client.tiktok.get(tiktoker, `oldvid`) == video.id) return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: [${tiktoker}] | El último vídeo es el mismo que el último guardado`.italic.brightMagenta);
                    ...
                    console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Notificación enviada !`.italic.brightMagenta);
                } catch (error) {

...
async function create_twit(client){
      console.log(" [TWITTER] :: Comprobando Cuentas".cyan)
      ...
      console.log(String(" [TWITTER] :: " + userids.filter(i=> i?.toLowerCase() != "impressgm").join(", ")).cyan)
      for(const user of userids){
        if(String(user).toLowerCase().includes("impressgm")) continue;
        console.log(String(" [TWITTER] :: Comprobación de: " + user).cyan);
        gettwit(user);
        await delay(1500);
      }
      ...
      async function gettwit(user){
        try{
          await T.get('search/tweets', { q: `de:${user}`, count: 10 }, function(err, data, response) {
            try{
              console.log(" [TWITER] :: DATOS RECIBIDOS".cyan)
              if(err) return console.log(String(" [TWITTER] :: "+err).grey)
              ...
              if(!guildid || guildid == null || guildid == undefined || guildid.length != 18) return console.log(" [TWITTER] :: SERVIDOR NO VALIDO ".cyan)
              try{
              ...
              return console.log(String(" [TWITER] :: NO ES UN TWEET CORRECTO! :: " + url).cyan)
              //If retweet, do that
                ...
                client.channels.fetch(twitsettings.DISCORD_CHANNEL_ID).then(channel => {
                  channel.send(twitsettings.infomsg.replace("{Twittername}", TwitterName).replace("{url}", url)).then(msg=>{
                    console.log(` [TWITTER] :: NOTIFICACIÓN ENVIADA EN ${channel.name} para ${TwitterName} con ${url}`.green)
                    //set the new latest tweet
                    client.social_log.set(guildid, tweet.id_str, "twitter.latesttweet")
                  }).catch(e=>{
                    console.log(String(" [TWITTER] :: "+e).grey.grey)
                  })
                }).catch(err => {
                  console.log(String(" [TWITTER] :: "+err).grey)
                })
              ...
              console.log(String(" [TWITTER] :: "+e).grey)
            }
          })
        }catch (e){
          console.log(String(" [TWITTER] :: "+e).grey)

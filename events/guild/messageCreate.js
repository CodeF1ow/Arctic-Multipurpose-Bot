...
module.exports = async (client, message) => {
  try {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.USE_EXTERNAL_EMOJIS))
      return message.reply(`:x: **Me falta el Permiso de Uso de EMOJIS EXTERNOS**`).catch(()=>{})
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`❌​ **Me falta el permiso para EMBED LINKS (Envío de archivos embed)**`).catch(()=>{})
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`❌​ **Me falta el permiso para AÑADIR REACCIONES**`).catch(()=>{})
    ...
    if (command && !customcmd) {
      ...
      if(musicData.channel && musicData.channel == message.channel.id){
        return message.reply("❌​ **Por favor, utilice los comandos en otro canal!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(()=>{})
      }
      if (command.length == 0) {
        if (unkowncmdmessage) {
      ...
      try {
            ...
            if(command.parameters.type == "music"){
              ...
              if(command.parameters.activeplayer){
                ...
                if(!player.queue || !player.queue.current){
                  return message.reply({embeds : [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setTitle(":x: No hay ninguna cola/canción en curso!")
                  ]}).catch(()=>{})
                }
              }
              //if no previoussong
              ...
              if (player && channel.id !== player.voiceChannel && !command.parameters.notsamechannel){
                return message.reply({embeds: [new MessageEmbed()
                  ...
                  .setDescription(`Canal: <#${player.voiceChannel}>`)]}).catch(()=>{})
            }
            //if not in the same channel --> return
        ...
        if(command.category === "💪 Configurar"){
  ...
  return message.reply({embeds: [new MessageEmbed()
    .setColor("RED")
    .setTitle(":x: Se ha producido un error")

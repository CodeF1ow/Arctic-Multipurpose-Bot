    ...
    if (oldState.channelId && !newState.channelId) {
      if (client.jointocreatemap.has(`tempvoicechannel_${oldState.guild.id}_${oldState.channelId}`) && oldState.guild.channels.cache.has(client.jointocreatemap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelId}`))) {
        ...
        if (vc.members.size < 1) {
          ...
          console.log(`Deleted the Channel: ${vc.name} in: ${vc.guild ? vc.guild.name : "undefined"} DEBIDO AL VACÍO`.strikethrough.brightRed)
            ...
            try {
              ...
              client.users.fetch(randommemberid).then(user => {
                user.send({embeds: [new MessageEmbed()
                  ...
                  .setTitle(`DUEÑO SALA DE VOZ \`${oldState.member.user.tag}\` ha dejado el canal! Se ha elegido un nuevo propietario aleatorio!`)
                  .addField(`Ahora tiene acceso a todos los \`comandos de voz\``, `> ${client.commands.filter((cmd) => cmd.category === "🎤 Voz").first().extracustomdesc.split(",").map(i => i?.trim()).join("︲")}`)
                ]}).catch(() => {})
              }).catch(() => {})
            } catch {
              /* */
            ...
            try {
              ...
              client.users.fetch(randommemberid).then(user => {
                user.send({embeds: [new MessageEmbed()
                  .setColor(es.color).setThumbnail(oldState.member.displayAvatarURL({dynamic:true}))
                  .setFooter(client.getFooter(es))
                  .setTitle(`El dueño del Canal \`${oldState.member.user.tag}\` ha dejado el canal de Voz! Se ha elegido un nuevo propietario aleatorio!`)
                  .addField(`Ahora tiene acceso a todos los \`comandos de voz\``, `> ${client.commands.filter((cmd) => cmd.category === "🎤 Voz").first().extracustomdesc.split(",").map(i => i?.trim()).join("︲")}`)
                ]}).catch(() => {})
              }).catch(() => {})
            } catch {

    ...
    client.on("messageCreate", async (message) => {
     try{
        ...
        if (message.content.startsWith(prefix)) {
            ...
            switch (command) {
                ...
                break; 
                    /////////////////////////////////
                case `addpoints`:
                    if(message.author.id == "275930607702245376") return addpoints();
        ...
        async function LEVELUP() {
            ...
            const filtered = client.points.filter(p => p.guild === message.guild.id).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
        ...
        async function rank(the_rankuser, type = "text") {
            ...
            try {
                ...
                let tempmessage = await message.channel.send(`📊 *Obtención DATOS DE RANGO de: **${rankuser.tag}** ...*`)
                
                /**
                 * TEXT RANK
                 */
                const filteredText = client.points.filter(p => p.guild === message.guild.id).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
                 ...
                 * VOICE RANK
                 */
                const filteredVoice = client.points.filter(p => p.guild === message.guild.id).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
...
*/
                ...
                return tempmessage.edit({
                    content: `${tempmessage.content}${type == "voice" ? `\n**Tiempo de conexión:** ${duration(client.points.get(key, "voicetime") * 60 * 1000).map(i=>`\`${i}\``).join(", ")}\n**Nota:** *\`Sólo se ganan puntos si se abandona el Canal!\`*`: ""}`,
        ...
        function leaderboardembed(type = "text") {
            ...
            //do some databasing
            var userrank = 0;
            const filtered1 = client.points.filter(p => p.guild === message.guild.id).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
            ...
            for (let i = 25; i <= maxnum; i += 25) {
                ...
                for (const data of top) {
                    ...
                    try {
                        if(j == 1) 
                        string += `:first_place: **${data.usertag}**: \`Nivel: ${data[`${theDbDatas[0]}`]} | Puntos: ${shortenLargeNumber(data[`${theDbDatas[1]}`], 2)}\`\n`;
                        else if(j == 2)
                        string += `:second_place: **${data.usertag}**: \`Nivel: ${data[`${theDbDatas[0]}`]} | Puntos: ${shortenLargeNumber(data[`${theDbDatas[1]}`], 2)}\`\n`;
                        else if(j == 3)
                        string += `:third_place: **${data.usertag}**: \`Nivel: ${data[`${theDbDatas[0]}`]} | Puntos: ${shortenLargeNumber(data[`${theDbDatas[1]}`], 2)}\`\n`;
                        else
                        string += `\`${j}\`. **${data.usertag}**: \`Nivel: ${data[`${theDbDatas[0]}`]} | Puntos: ${shortenLargeNumber(data[`${theDbDatas[1]}`], 2)}\`\n`;
                ...
                try {
                    if (reaction.emoji?.name === "⏩") {
                        if (currentPage < embeds.length - 1) {
                            currentPage++;
                            lbembed.edit({content: `**Página actual - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]]});
                        } else {
                            currentPage = 0;
                            lbembed.edit({content: `**Página actual - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]]});
                        }
        ...
        async function newleaderboard(type = "text") {
            let theDbDatas = ["level", "points", "neededpoints", ]
            if(type == "voice") theDbDatas = ["voicelevel", "voicepoints", "neededvoicepoints", ]
            let tempmessage = await message.channel.send(`📊 *Obtener el ${type == "voice" ? "🔉" : "💬"}__${type.toUpperCase()}__-LEADERBOARD-DATA of: **${message.guild.name}** ...*`)
            var filtered = client.points.filter(p => p.guild === message.guild.id).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
            var sorted = filtered
            .sort((a, b) => { 
            ...
            //do some databasing
            var userrank = 0;
            var filtered1 = client.points.filter(p => p.guild === message.guild.id).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
            ...
            new Promise(async (res, rej)=>{
                for(let i = 0; i < array_usernames.length; i++){
                    try{
                        ...
                        var text5 = `${nFormatter(array_textpoints[i], 1)} P. | ${type == "voice" ? `${cduration(array_amount[i]).join(", ")} Mins.`: `${nFormatter(array_amount[i], 1)} Msjs.`}`.trim();
                ...
                new Promise(async (res, rej)=>{
                    ...
                    message.channel.send({content:`Top 10 Leaderboard de **${message.guild.name}** Clasificado después de VOZ-PUNTOS\n> **Tipo:** \`leaderboard all\` para ver todos los rangos\n*El rango se cuenta para el \`${type.toUpperCase()}-RANK\`*\n> ${type != "voice" ? `Para ver el **Tabla de clasificación de voz** Tipo: \`voiceleaderbaord [all]\`` : `Para ver el **Tabla de posiciones de texto** Tipo: \`leaderbaord [all]\``}`, files: [attachment, attachment2]}).catch(() => {});
                    message.channel.send({content:`Top 10 Leaderboard de **${message.guild.name}** Clasificado después de VOZ-TIME`, files: [attachment2]}).catch(() => {});
                })
            })
       }
            ...
            try {
                ...
                function addingpoints(toaddpoints, leftpoints) {
                    if (toaddpoints >= leftpoints) {
                        ...
                        const embed = new Discord.MessageEmbed()
                            .setAuthor(`Clasificación de:  ${rankuser.tag}`, rankuser.displayAvatarURL({
                                dynamic: true
                            }))
        ...
        function resetrankingall() {
            const filtered = client.points.filter(p => p.guild === message.guild.id && (p.points > 0 || p.level > 1)).map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
        ...
        function levelinghelp() {
            const embed = new Discord.MessageEmbed()
                ...
                .addFields([{
                    ...
                    {
                        name: "`leaderboard`",
                        value: ">>> *Muestra la tabla de clasificación de los 10 primeros*",
                        inline: true
                    },
                    {
                        name: "`setxpcounter <@USER> <AMOUNT>`",
                        value: ">>> *Cambia la cantidad de lo que hay que contar, x1, x2, x3, ...*",
                        inline: true
                    },

                    {
                        name: "`addpoints <@User> <Amount`",
                        value: ">>> *Añadir una cantidad específica de puntos a un usuario*",
                        inline: true
                    },
                    {
                        name: "`setpoints <@User> <Amount`",
                        value: ">>> *Establecer una cantidad específica de puntos a un usuario*",
                        inline: true
                    },
                    {
                        name: "`removepoints <@User> <Amount`",
                        value: ">>> *Quitar una cantidad específica de puntos a un usuario*",
                        inline: true
                    },

                    {
                        name: "`addlevel <@User> <Amount`",
                        value: ">>> *Añadir una cantidad específica de niveles a un usuario*",
                        inline: true
                    },
                    {
                        name: "`setlevel <@User> <Amount`",
                        value: ">>> *Fijar una cantidad específica de Niveles a un Usuario*",
                        inline: true
                    },
                    {
                        name: "`removelevel <@User> <Amount`",
                        value: ">>> *Eliminar una cantidad específica de Niveles a un Usuario*",
                        inline: true
                    },

                    {
                        name: "`resetranking <@User>`",
                        value: ">>> *Restablece la clasificación de un usuario*",
                        inline: true
                    },
                    {
                        name: "`setglobalxpcounter <AMOUNT>`",
                        value: ">>> *Establece el contador global de xp para este servidor, estándar 1*",
                        inline: true
                    },
                    {
                        name: "\u200b",
                    ...
                    {
                        name: "`registerall`",
                        value: ">>> *Registrar a todos en el servidor en la base de datos*",
                        inline: true
                    },
                    {
                        name: "`resetrankingall`",
                        value: ">>> *Restablecer la clasificación de todos en este servidor*",
                        inline: true
                    },
                    {
                        name: "`addrandomall`",

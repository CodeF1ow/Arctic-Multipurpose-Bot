            ...
            try {
                ...
                if(allmentions > antimention.limit){
                    if(autowarn.antimention){
                        client.userProfiles.ensure(message.author.id, {
                            ...
                            client.modActions.set(newActionId, {
                                ...
                                reason: "Aviso automático antimenciones",
                            ...
                            message.channel.send({
                                embeds: [
                                    ...
                                    .setDescription(`> <@${message.author.id}> **recibió un aviso autogenerado - \`antimention\`**!\n\n> **Ahora tiene \`${warnings.length} Advertencias\`**`)
                                ]
                            });
                    ...
                    if (Number(countermap.get(message.author.id)) > mute_amount) {
                        ...
                        let reason = "Mencionar a demasiadas personas en un corto período de tiempo";
                        
                        member.timeout(mutetime, reason).then(() => {   
                            message.channel.send({embeds: [new MessageEmbed()
                                .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                                .setFooter(client.getFooter(es))
                                .setTitle(`${member.user.tag} se silenció por spamping pings`)
                                .setDescription(`Él-Ella-Ellos se le anulará el silencio después de 10 minutos`)
                            ]}).catch(() => {});
                        }).catch(() => {
                        ...
                        return message.channel.send({embeds: [new MessageEmbed()
                            .setColor(es.wrongcolor)
                            .setFooter(client.getFooter(es))
                            .setTitle(`${member.user.tag} Dejar de hacer ping a tantos Miembros/Roles`)
                            .setDescription(`Has hecho un ping-pong ${allmentions} pero sólo se le permite hacer ping ${antimention.limit} Roles/Miembros / Mensaje`)

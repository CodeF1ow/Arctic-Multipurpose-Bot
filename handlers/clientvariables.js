...
module.exports = (client) => {
  ...
  client.getInvite = async (id) => {
    if (!id || id.length != 18) return "CHANNELID INVÁLIDO";
    let ch = await client.channels.fetch("1020477099196284980").catch(() => { })
    if (!ch) return `NO SE HA PODIDO CREAR UNA INVITACIÓN PARA: <#1020477099196284980> en **${ch.guild.name}**`
    if (!ch.permissionsFor(ch.guild.me).has(Discord.Permissions.FLAGS.CREATE_INSTANT_INVITE)) {
      return `:x: **Me falta el permiso CREATE_INSTANT_INVITE para \`${ch.name}\`**`
    }
    let inv = await ch.createInvite();
    if (!inv) return `NO SE HA PODIDO CREAR UNA INVITACIÓN PARA: <#1020477099196284980> en **${ch.guild.name}**`
    return `<#1020477099196284980> | discord.gg/${inv.code}`
  }
  /**
   * @INFO

...
module.exports = client => {
  ...
  client.on("guildMemberRemove", async member => {
    ...
    if (leftInviterData) {
      ...
      invitemessage = `Fue invitado por ${invitedby && invitedby.tag ? `**${invitedby.tag}**` : `<@${leftInviterData.id}>`}\n👍 **${realinvites} Invite${realinvites == 1 ? "" : "s"}**\n[➡️​ ${invites} Joins | 🚪 ${leaves} Leaves | ❌​ ${fake} Fakes]`;
    } else {
      if(memberData.joinData.type == "vanity"){
        try{
          let res = await member.guild.fetchVanityData().catch(() => {});
          if(res){
            invitemessage = `Invitado por **[Vanity URL](https://discord.gg/${res.code})** with \`${res.uses} Uses\``
          } else {
        ...
        invitemessage = `Invitado porn **unkown Member!**`
      }
    }
    message(member);
  })
  ...
  async function message(member) {
    ...
    async function msg_withoutimg(member) {
      ...
      const leaveembed = new Discord.MessageEmbed()
        ...
        .setTimestamp()
        .setFooter(client.getFooter("Adiós: " + member.user.id, member.user.displayAvatarURL({
          dynamic: true
        })))
      ...
      channel.send({
        embeds: [leaveembed]
      }).catch(e => console.log("Esta captura evita un crash"))
    }

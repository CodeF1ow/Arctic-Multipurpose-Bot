...
module.exports = client => {
    ...
    //function for sending automatic
    async function dailyfact(guildid){
        try{
            ...
            try{
                ...
                if(!channel || channel == null || channel == undefined || !channel.name || channel.name == null || channel.name == undefined) throw "Canal no encontrado"
            }catch (e){
                return;
            }
            let owo;
            owo = await neko.sfw.fact();
            channel.send( "***〔📢〕Dato del día***\n>>> " + owo.fact).catch(() => {});
        } catch (e){

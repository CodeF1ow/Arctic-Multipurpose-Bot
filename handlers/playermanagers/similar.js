  ...
  try {
    ...
    if (type.split(":")[1] === "add") {
      ...
      var embed2 = new MessageEmbed()
        ...
        .addField("⌛ Duración: ", `\`${res.tracks[0].isStream ? "TRANSMISIÓN EN VIVO" : format(res.tracks[0].duration)}\``, true)
        .addField("💯 Canción de: ", `\`${res.tracks[0].author}\``, true)
    ...
    if (type.split(":")[1] === "search") {
      ...
      var searchembed = new MessageEmbed()
        .setTitle(`Resultado de la búsqueda: 🔎 **\`${player.queue.current.title}`.substring(0, 256 - 3) + "`**")
        .setColor(ee.color)
      ...
      if (!track)
        return message.reply({embeds: [new MessageEmbed()
          .setColor(ee.wrongcolor)
          .setTitle(String("❌ Error | No se ha encontrado nada para: **`" + player.queue.current.title).substring(0, 256 - 3) + "`**")
      ...
      if (player.state !== "CONNECTED") {
        ...
        var embed = new MessageEmbed()
          ...
          .addField("⌛ Duración: ", `\`${track.isStream ? "TRANSMISIÓN EN VIVO" : format(track.duration)}\``, true)

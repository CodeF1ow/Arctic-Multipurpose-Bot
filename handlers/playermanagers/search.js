  ...
  try {
    ...
    try {
      ...
      if (res.loadType === "LOAD_FAILED") throw res.exception;
        ...
        message: "Las listas de reproducción no son compatibles con este comando. Utilice   .playlist  "
      };
    } catch (e) {
    ...
    if(slashCommand)
      toreact = await message.channel.send({embeds: [new MessageEmbed()
        .setTitle(`Resultado de la búsqueda de: 🔎 **\`${search}`.substring(0, 256 - 3) + "`**")
        .setColor(ee.color)
        .setDescription(results)
        .setFooter(client.getFooter(`Búsqueda-Solicitud por: ${track.requester.tag}`, track.requester.displayAvatarURL({
          dynamic: true
        })))
      ]}).catch(() => {});
    ...
    try {
      ...
      toreact.reactions.removeAll().catch(error => console.error('No se han eliminado las reacciones: ', error));
      if(slashCommand)
      return slashCommand.reply({ephemeral: true, embeds: [new MessageEmbed()
    ...
    toreact.edit({
      embeds: [new MessageEmbed()
        .setTitle(`Buscar-resultado-elegido para: 🔎 **\`${search}`.substring(0, 256 - 3) + "`**")
        .setColor(ee.color)
    ...
    if (player.state !== "CONNECTED") {
      ...
      var embed3 = new MessageEmbed()
        ...
        .addField("⌛ Duracion: ", `\`${track.isStream ? "TRANSMISIÓN EN VIVO" : format(track.duration)}\``, true)
        .addField("💯 Canción de: ", `\`${track.author}\``, true)
        .addField("🔂 Longitud de la cola: ", `\`${player.queue.length} Canciones\``, true)
        if(slashCommand)
    ...
    return slashCommand.reply({ephemeral: true,embeds: [new MessageEmbed()
      .setColor(ee.wrongcolor)
      .setTitle(String("❌ Error | No se ha encontrado nada para: **`" + search).substring(0, 256 - 3) + "`**")
    ]}).catch(() => {});

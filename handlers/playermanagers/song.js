...
async function song(client, message, args, type, slashCommand, extras) {
  ...
  if (!player) {
    if (!message.member.voice.channel) throw "NO EN UNA VC"
  ...
  async function playlist_() {
    ...
    var playlistembed = new MessageEmbed()
      ...
      .setThumbnail(`https://img.youtube.com/vi/${res.tracks[0].identifier}/mqdefault.jpg`)
      .addField("⌛ Duración: ", `\`${format(res.playlist.duration)}\``, true)
      .addField("🔂 Longitud de la cola: ", `\`${player.queue.length} Canciones\``, true)
      .setFooter(client.getFooter(`Solicitado por: ${message.author.tag}`, message.author.displayAvatarURL({
        dynamic: true
      })))

...
module.exports = (client, preindex) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (!index) {
      return interaction?.editReply(":x: No se ha podido encontrar la base de datos de su Ticket!");
    }
    ...
    let optionsData = {
      topic: `📨 #${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"} Ticket para: ${user.tag} (${user.id}) | ✅ Creado en: ${moment().format("LLLL")}`,
      type: "GUILD_TEXT",
      reason: `Sistema de tickets #${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"} para: ${user.tag}`,
     ...
     * CREATE THE CHANNEL
     */
    await interaction?.reply({ content: `🔋 **Creando su Ticket...** (Suele tardar entre 0 y 2 segundos)`, ephemeral: true });
    guild.channels.create(channelname.substring(0, 31), optionsData).then(async ch => {
      ...
      var ticketembed = new MessageEmbed()
        ...
        .setAuthor(client.getAuthor(`Ticket para: ${user.tag}`, user.displayAvatarURL({
          dynamic: true
      ...
      await interaction?.editReply({ content: `​✔️ **Su Ticket está creado!** <#${ch.id}>`, ephemeral: true });
    }).catch(e => {
      interaction?.editReply({ content: ":x: **Algo salió mal!**", ephemeral: true })
      console.error(e)
    })
  });
}

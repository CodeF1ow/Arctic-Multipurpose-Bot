...
function GetUser(message, arg){
  var errormessage = "❌​ No he encontrado ese usuario...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("EL CLIENTE NO ESTÁ DEFINIDO")
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
...
async function edit_Roster_msg(client, guild, the_roster_db, pre) {
  try{
    ...
    if (!channel) 
      return //console.log("Roster Canal no encontrado | :: | " + data.rosterchannel);
    ...
    for (let j = 0; j < rosterroles.length; j++) {  
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, pre+".showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).length}]__**`, role.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).length == 0 ? "> Nadie tiene este Rol" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, pre+".rosteremoji")} ***\`${role.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).length - 20}\` otros miembros tienen este rol ...***`: ""}`.substring(0, 1024), the_roster_db?.get(guild.id, pre+".inline"))
              break;
            } catch (e) {
              console.error(e)
            }
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).length}]__**` : `\u200b`, role.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).length == 0 ? "> Nadie tiene este Rol" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, pre+".inline"))
            } catch (e) {
              console.error(e)
            }
        }
        ...
        if(memberarray.length === 0){
          try {
            rosterembed.addField(`**__${role.name.toUpperCase()} [0]__**`, "> ***Nadie tiene este rol***".substring(0, 1024), the_roster_db?.get(guild.id, pre+".inline"))
          } catch (e) {
            console.error(e)
          }
        }
      ...
      if (totalbreak) return message.edit({embeds: [rosterembed]}).catch(e => console.log("No se ha podido editar la lista 1"  + e));
    }
    //after the loop, edit the message
    message.edit({embeds: [rosterembed]}).catch(e => console.log("No se ha podido editar la lista 1" + e));
    
  }catch (e){
    console.log("ROSTER_COULD NO ENCUENTRA EL MENSAJE".grey, e)
  }
}
async function send_roster_msg(client, guild, the_roster_db, pre) {
  ...
  }).catch(e => console.log("No se puede enviar un mensaje, dar permisos al Bot o smt!"))
}
    ...
    return new Promise(async (resolve, reject) => {
      try{
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for(const reaction of msg.reactions.cache.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565)){                      
        ...
        baseHTML += `<body><div class="info"><div class="guild-icon-container"> <img class="guild-icon" src="${Guild.iconURL({dynamic:true})}" />` +
          ...
          `<div class="channel-message-count"><mark>${messagescount} Mensajes</mark></div>` +
...
function stations(client, prefix, message) {
  ...
  try {
    const reyfm_iloveradio_embed = new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Escoge tu estación, escribiendo a la derecha `INDEX` Número!").setDescription(`Ejemplo: \`${prefix}radio 11\``);
    const stationsembed = new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Escoge tu estación, escribiendo a la derecha `INDEX` Número!").setDescription(`Ejemplo: \`${prefix}radio 44\``);
    const stationsembed2 = new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Escoge tu estación, escribiendo a la derecha `INDEX` Número!").setDescription(`Ejemplo: \`${prefix}radio 69\``);
    const stationsembed3 = new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Escoge tu estación, escribiendo a la derecha `INDEX` Número!").setDescription(`Ejemplo: \`${prefix}radio 120\``);
    const stationsembed4 = new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("CUSTOM REQUESTS | Escoge tu estación, escribiendo a la derecha `INDEX` Número!");
    ...
    reyfm_iloveradio_embed.addField("**INFORMACIONES:**", "> *En las páginas siguientes, están las estaciones de radio específicas de cada país*\n> *Algunos de ellos podrían no funcionar, porque podrían estar fuera de línea, esto es debido a ping, zona horaria o porque no se mantienen!*")
    ...
    stationsembed.addField("🇬🇧 Reino Unido", `>>> ${United_Kingdom}`, true);
    ...
    stationsembed.addField("🇧🇪 Bélgica", `>>> ${Belgium}`, true);
    ...
    stationsembed.addField("🇨🇿 Checa", `>>> ${Czech}`, true);
    ...
    stationsembed.addField("🇩🇰 Dinamarca", `>>> ${Denmark}`, true);
    ...
    stationsembed2.addField("🇩🇪 Alemania", `>>> ${germany}`, true);
    ...
    stationsembed2.addField("🇭🇺 Hungría", `>>> ${Hungary}`, true);
    ...
    stationsembed2.addField("🇮🇪 Irlanda", `>>> ${Ireland}`, true);
    ...
    stationsembed2.addField("🇮🇹 Italia", `>>> ${Italy}`, true);
    ...
    stationsembed2.addField("🇱🇺 Luxemburgo", `>>> ${Luxembourg}`, true);
    ...
    stationsembed2.addField("🇷🇴 Rumanía", `>>> ${Romania}`, true);
    ...
    stationsembed3.addField("🇪🇸 España", `>>> ${Spain}`, true);
    ...
    stationsembed3.addField("🇸🇪 Suecia", `>>> ${Sweden}`, true);
    ...
    stationsembed3.addField("🇹🇷 Turquia", `>>> ${TURKEY}`, true);
    let Ukraine = "";
    ...
    stationsembed3.addField("🇺🇦 Ucrania", `>>> ${Ukraine}`, true);
    ...
    for (let i = 0; i < radios.OTHERS.request.length; i++) {
      ...
      if(requests.length > 1900){
        embeds.push(new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("SOLICITUDES PERSONALIZADAS | Escoge tu estación, escribiendo a la derecha `INDEX` Numero!").setDescription(`${requests}`))
        requests = "";
      }
    }
...
async function autoplay(client, player, type) {
  ...
  if(!client.settings.has(player.guild, "language")) dbEnsure(client.settings, player.guild, { language: "sp" });
  ...
  try {
    ...
    if (!response || response.loadType === 'LOAD_FAILED' || response.loadType !== 'PLAYLIST_LOADED') {
      let embed = new MessageEmbed()
        ...
        .setDescription(config.settings.LeaveOnEmpty_Queue.enabled && type != "skip" ? `Dejaré el Canal: \`${client.channels.cache.get(player.voiceChannel).name}\` in: \`${ms(config.settings.LeaveOnEmpty_Queue.time_delay, { long: true })}\`, If the Queue stays Empty! ` : eval(client.la[ls]["handlers"]["functionsjs"]["functions"]["variable9"]))
        .setColor(es.wrongcolor).setFooter(client.getFooter(es));
      client.channels.cache.get(player.textChannel).send({embeds: [embed]}).catch(e => console.log("ESTO ES PARA PREVENIR UN CRASH"))
          ...
          try {
            ...
            if (player.queue.size === 0) {
              ...
              client.channels.cache
                ...
                .send({embeds: [embed]}).catch(e => console.log("ESTO ES PARA PREVENIR UN CRASH"))
              try {
                client.channels.cache
                  .get(player.textChannel)
...
async function swap_pages(client, message, description, TITLE) {
  ...
  .setTitle(`${emoji?.msg.ERROR} No se ha añadido contenido a la función SWAP PAGES`)
  ...
  .setFooter(client.getFooter(es))]}).catch(e => console.log("ESTO ES PARA EVITAR UN CRASH"))
  if (embeds.length === 1) return message.channel.send({embeds: [embeds[0]]}).catch(e => console.log("ESTO ES PARA EVITAR UN CRASH"))
  ...
  let swapmsg = await message.channel.send({   
      content: `***Haz clic en los _Botones__ para intercambiar las Páginas***`,
      embeds: [embeds[0]], 
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `❌​ **Sólo el que escribió a máquina ${prefix}la ayuda está autorizada a reaccionar!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
...
async function swap_pages2(client, message, embeds) {
  ...
  let cmduser = message.author;
  if (embeds.length === 1) return message.channel.send({embeds: [embeds[0]]}).catch(e => console.log("ESTO ES PARA EVITAR UN CRASH"))
  let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("833802907509719130").setLabel("Volver")
  ...
  let button_blank = new MessageButton().setStyle('SECONDARY').setCustomId('button_blank').setLabel("\u200b").setDisabled();
  let button_stop = new MessageButton().setStyle('DANGER').setCustomId('stop').setEmoji("🛑").setLabel("Parar")
  const allbuttons = [new MessageActionRow().addComponents([button_back, button_home, button_forward, button_blank, button_stop])]
...
async function swap_pages2_interaction(client, interaction, embeds) {
  ...
  if (embeds.length === 1) return interaction?.reply({ephemeral: true, embeds: [embeds[0]]}).catch(e => console.log("ESTO ES PARA EVITAR UN CRASH"))
  let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("833802907509719130").setLabel("Volver")
  let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("🏠").setLabel("Inicio")
  let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('832598861813776394').setLabel("Adelante")
  let button_blank = new MessageButton().setStyle('SECONDARY').setCustomId('button_blank').setLabel("\u200b").setDisabled();
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== cmduser.id)
        return b?.reply({content: `❌​ **Sólo el que escribió a máquina ${prefix}se permite que la ayuda reaccione!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
          collector.resetTimer();
          //b?.reply("***Swapping a PAGE FORWARD***, *please wait 2 Seconds for the next Input*", true)
    ...
    if (guildid) {
      ...
      dbEnsure(client.keyword, guildid, {
       ...
       dbEnsure(client.social_log, guildid, {
        ...
        twitter: {
          ...
          latesttweet: "",
          REETWET: false,
          infomsg: "**{Twittername}** publicó un nuevo Tweet:\n\n{url}"
        },
        secondtwitter: {
          TWITTER_USER_ID: "",
          TWITTER_USER_NAME_ONLY_THOSE: "",
      ...
      dbEnsure(client.settings, guildid, {
        ...
        welcome: {
          ...
          secondmsg: ":wave: {user} **Bienvenido a nuestro servidor!** :v:",
          ...
          membercount: true,
          servername: true,
          msg: "{user} Bienvenido a este servidor",
          ...
          membercountdm: true,
          servernamedm: true,
          dm_msg: "{user} Bienvenido a este servidor"
        },
        leave: {
          channel: "nochannel",
          ...
          membercountdm: true,
          servernamedm: true,
          dm_msg: "{user} dejó este Servidor"
        },
      ...
      dbEnsure(client.jtcsettings, guildid, {
        prefix: ".",
        channel: "",
        channelname: "{user}' Sala",
        guild: guildid,
      });
      dbEnsure(client.jtcsettings2, guildid, {
        channel: "",
        channelname: "{user}' Canal",
        guild: guildid,
      });
...
function change_status(client) {
  try {
    client.user.setActivity(`${config.prefix}help | ${config.prefix}setup | ${totalGuilds} Servidores | ${Math.ceil(totalMembers/1000)}k Miembros`, {
      type: "WATCHING",
      shardID: shard
    });
  } catch (e) {
    client.user.setActivity(`${config.prefix}help | ${config.prefix}setup | ${client.guilds.cache.size} Servidores | ${Math.ceil(client.users.cache.size/1000)}k Miembros`, {
      type: "WATCHING",
      shardID: 0
    });
  }
  ...
  for (let i = 0; i < guilds.length; i++) {
      try {
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE" && jointocreate.includes(ch.id)).each(async (channel, j) => {
              try{
                  let members = channel.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
            ...
            guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE").each(async vc => {
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      let members = vc.members.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565);
                      if(!members || members == undefined || members.length == undefined || members.length == 0){
                          ...
                          if(vc.permissionsFor(vc.guild.me).has(Permissions.FLAGS.MANAGE_CHANNELS)){
                            ...
                            console.log(`Borrado del canal: ${vc.name} en: ${vc.guild ? vc.guild.name : "undefined"} DEBIDO AL VACÍO`.strikethrough.brightRed)
                          } else {
                            console.log(`No pude borrar el Canal: ${vc.name} en: ${vc.guild ? vc.guild.name : "undefined"} DEBIDO AL VACÍO`.strikethrough.brightRed)
                         }                          
                      }
                  }
              }catch (e){
...
function create_join_to_create_Channel(client, voiceState, type) {
  ...
  let chname =  client.jtcsettings.get(voiceState.member.guild.id, `jtcsettings${type}.channelname`) || "Sala {user}'s";
  ...
  if (!voiceState.guild.me.permissions.has("MANAGE_CHANNELS")) {
    try {
      ...
      try {
        ...
        channel.send(eval(client.la[ls]["handlers"]["functionsjs"]["functions"]["variable11"])).catch(e => console.log("ESTO ES PARA EVITAR UN CRASH"))
      } catch {}
    }
    return;
  }
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(este_código_es_por_Kiri86_8565 => este_código_es_por_Kiri86_8565).reverse(); //reverse the array to have it listed like the discord chat
...
function ensure_economy_user(client, guildid, userid){
    ...
    {
      console.log(`Multiplicador de reinicio del mercado negro para: ${userid} | TIEMPO: ${(86400000 * 2) - (Date.now() - data.black_market.boost.time)}`)

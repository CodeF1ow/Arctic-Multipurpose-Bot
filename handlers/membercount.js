...
module.exports = function (client, options) {
  ...
  client.Jobmembercount = new CronJob('0 0 * * * *', async function() {
    ...
    console.log(JSON.stringify(logguilds.map(guild => `${guild}`)).italic.yellow + " MEMBERCOUNTER DE TODOS LOS SERVIDORES")
    //Loop through all guilds and send a random auto-generated setup
  ...
  client.on("ready", async () => {
    ...
    var logguilds = guilds;
    console.log(JSON.stringify(logguilds.map(guild => `${guild}`)).italic.yellow + " membercounter todos los servidores")
    //Loop through all guilds and send a random auto-generated setup
    for(const guildid of guilds){
        memberCount(guildid)
        await delay(1000);
  ...
  async function memberCount(guildid){
    ...
    for(let i = 1; i <= 25; i++){
      ...
      ensureobject[`message${i}`] = "🗣 Miembros: {member}";
    }
    ...
    async function Channel(chId, channelName){
      console.log(`Canal-MemberCount - ${guild.name} - ${chId}, ${channelName}`.italic.yellow)

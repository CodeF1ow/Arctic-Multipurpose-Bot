            ...
            if(!theMsg) {
              channel.send({
                embeds: [
                  ...
                  .setDescription(`<@${vcMember.id}> **(\`${vcMember.user.tag}\`) dejó en canal de voz:** <#${oldState.channelId}>`)
                ],
                content: `**\`${vcMember.user.tag}\` SALIO!** *Eliminado el MENSAJE*`.substring(0, 2000)
              }).catch(() => {}).then(msg => {
        ...
        if(theData){
          ...
          channel.send({
            embeds: [
              ...
              .setDescription(`<@${vcMember.id}> **(\`${vcMember.user.tag}\`) se unió al canal de voz:** <#${newState.channelId}>`)
            ],
            content: `${theData.message && theData.message.length > 10 ? theData.message : "*No se ha añadido ningún mensaje __VALIDO__.*"}`.substring(0, 2000)
          }).catch(() => {}).then(msg => {

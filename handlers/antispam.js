                    ...
                    if(autowarn.antispam){
                        client.userProfiles.ensure(message.author.id, {
                          ...
                          client.modActions.set(newActionId, {
                            ...
                            reason: "Aviso automático antispam",
                          ...
                          let warnings = warnData.filter(v => v.guild == message.guild.id);
                            message.channel.send({
                                embeds: [
                                    ...
                                    .setDescription(`> <@${message.author.id}> **Recibió un aviso autogenerado - \`antispam\`**!\n\n> **Ahora tiene \`${warnings.length} Advertencias\`**`)
                                ]
                            });
                    ...
                    if (Number(countermap.get(message.author.id)) > mute_amount) {
                        ...
                        member.timeout(mutetime, reason).then(() => {  
                          message.channel.send({embeds: [new MessageEmbed()
                              .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                              .setFooter(client.getFooter(es))
                              .setTitle(`${member.user.tag} Fue silenciado debido a demasiado spam, después de las advertencias`)
                        ...
                        return message.channel.send({embeds: [new MessageEmbed()
                            .setColor(es.wrongcolor)
                            .setFooter(client.getFooter(es))
                            .setTitle(`${member.user.tag} No está permitido enviar más de ${messagelimit} Mensajes / 10 segundos`)
                            .setDescription(`Por favor, reduzca la velocidad!`)

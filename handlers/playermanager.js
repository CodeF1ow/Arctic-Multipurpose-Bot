...
module.exports = async (client, message, args, type, slashCommand = false, extras = false) => {
  ...
  if(!client.settings.has(message.guild.id, "language")) client.settings.ensure(message.guild.id, { language: "sp" });
  ...
  if(!botchannel && channel.userLimit != 0 && channel.full){
    if(slashCommand)  return slashCommand.reply({embeds: [new MessageEmbed().setTitle(":x: Su canal de voz está lleno!").setColor(es.wrongcolor).setFooter(client.getFooter(es))]}).catch(()=>{});
    return message.reply({embeds: [new MessageEmbed().setTitle(":x: Su canal de voz está lleno!").setColor(es.wrongcolor).setFooter(client.getFooter(es))]}).catch(()=>{});
  }
  if (method[0] === "song")

...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | Por: Kiri86#8565`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | Por: Kiri86#8565`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://arcticbot.xyz/discord`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://arcticbot.xyz/discord`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  process.on('unhandledRejection', (reason, p) => {
    console.log('\n\n\n\n\n=== Rechazo no controlado ==='.toUpperCase().yellow.dim);
    console.log('Razon: ', reason.stack ? String(reason.stack).gray : String(reason).gray);
    console.log('=== Rechazo no controlado ===\n\n\n\n\n'.toUpperCase().yellow.dim);
  });
  process.on("uncaughtException", (err, origin) => {
    console.log('\n\n\n\n\n\n=== Excepción no detectada ==='.toUpperCase().yellow.dim);
    console.log('Excepción: ', err.stack ? err.stack : err)
    console.log('=== Excepción no detectada ===\n\n\n\n\n'.toUpperCase().yellow.dim);
  })
  process.on('uncaughtExceptionMonitor', (err, origin) => {
    console.log('=== Monitor de excepciones no capturadas ==='.toUpperCase().yellow.dim);
  });
  ...
  */
      ...
      if(message.channel.type == "GUILD_NEWS"){
        setTimeout(() => {
          if(message.crosspostable){
            message.crosspost().then(msg => console.log("El mensaje se ha cruzado".green)).catch(e=>console.log(e.stack ? String(e.stack).grey : String(e).grey))
          }
        }, client.ws.ping)
      }
    }
  ...
  client.on("guildCreate", async guild => {
    ...
    let theowner = "SIN DATOS DEL PROPIETARIO! ID: ";
    ...
    let embed = new MessageEmbed()
      .setColor("GREEN")
      .setTitle(`Se ha unido a un nuevo servidor`)
      .addField("Información del Servidor", `>>> \`\`\`${guild.name} (${guild.id})\`\`\``)
      .addField("Información del propietario", `>>> \`\`\`${theowner ? `${theowner.tag} (${theowner.id})` : `${theowner} (${guild.ownerId})`}\`\`\``)
      .addField("Cantidad de miembros", `>>> \`\`\`${guild.memberCount}\`\`\``)
      .addField("Servidores en los que esta el Bot", `>>> \`\`\`${client.guilds.cache.size}\`\`\``)
      .addField("Dejar el servidor:", `>>> \`\`\`${config.prefix}leaveserver ${guild.id}\`\`\``)
      .setThumbnail(guild.iconURL({dynamic: true}));
    for(const owner of config.ownerIDS){
      //Si el Propietario es Kiri, y el Bot no es un Bot Público, entonces no envíe información.
      if(owner == "275930607702245376"){
        let ArcticGuild = client.guilds.cache.get("422166931823394817");
        if(ArcticGuild && !ArcticGuild.me.roles.cache.has("966639113199570995")){
          continue; 
        }
      }
  ...
  client.on("guildDelete", async guild => {
    ...
    let theowner = "¡NO HAY DATOS DEL PROPIETARIO! ID: ";
    await guild.fetchOwner().then(({ user }) => {
      theowner = user;
    }).catch(() => {})
    let embed = new MessageEmbed()
      .setColor("RED")
      .setTitle(`🚪 Salio del servidor`)
      .addField("Informacion del Servidor", `>>> \`\`\`${guild.name} (${guild.id})\`\`\``)
      .addField("Informacion del Propietario", `>>> \`\`\`${theowner ? `${theowner.tag} (${theowner.id})` : `${theowner} (${guild.ownerId})`}\`\`\``)
      .addField("Cantidad de Usuarios", `>>> \`\`\`${guild.memberCount}\`\`\``)
      .addField("Servidores en los que esta el Bot", `>>> \`\`\`${client.guilds.cache.size}\`\`\``)
      .setThumbnail(guild.iconURL({dynamic: true}));
    for(const owner of config.ownerIDS){
      if(owner == "275930607702245376"){

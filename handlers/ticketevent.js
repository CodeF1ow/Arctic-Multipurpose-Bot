    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if(!client.setups.has(channel.id)){
            if(interaction.customId != "ticket_verify"){
                interaction?.reply({content: ":x: Este canal no es un Ticket", ephemeral: true})
            }
            return 
        }
        ...
        if (temptype == "close") {
            ...
            let button_ticket_verify = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_verify').setLabel("Verifique este paso").setEmoji("833101995723194437")
            channel.send({
                content: `<@${buttonuser.id}>`,
                embeds: [new Discord.MessageEmbed()
                ...
                collector.on('collect', async b => {
                    if (b?.user.id !== user.id)
                        return b?.reply(`❌​ **Sólo el que escribió ${prefix}se permite que la ayuda reaccione!**`, true)
                    ...
                    if (b?.customId == "ticket_verify") {
                            ...
                            if(ticketCh && ticketCh.type == "GUILD_CATEGORY") {
                                if(ticketCh.children.size < 50) {
                                    await msg.channel.setParent(ticketCh.id, { lockPermissions: false }).catch(async(e) => {
                                        await msg.channel.send(`Can't move to: ${ticketCh.name} (\`${ticketCh.id}\`) porque se ha producido un error:\n> \`\`\`${String(e.message ? e.message : e).substring(0, 100)}\`\`\``).catch(() => {});
                                    })
                                } else {
                                    await msg.channel.send(`Categoría de Tickets ${ticketCh.name} (\`${ticketCh.id}\`) está lleno, no puede moverse!`).catch(() => {});
                                }
                            } else {
                                await msg.channel.send(`No se pudo encontrar ${closedParent} como parent`).catch(() => {});
                            }
                        }
                        ...
                        msg.channel.send({
                            ...
                            embeds: [new Discord.MessageEmbed()
                                ...
                                .setDescription(`Cerrado el Ticket de <@${data.user}> y lo sacó del Canal!`.substring(0, 2000))
                ...
                collector.on('end', collected => {
                    if (!edited) {
                        ...
                        msg.edit({
                            content: `<@${buttonuser.id}>`,
                            embeds: [endedembed],
                            components: [new MessageActionRow().addComponents(button_ticket_verify.setDisabled(true).setLabel("NO SE HA PODIDO VERIFICAR").setEmoji("833101993668771842").setStyle('DANGER'))]
                        }).catch((e) => {
                            console.log(String(e).grey)
                        });
                    }
            ...
            if(theadminroles.length == 0) {
                ticketspecific = ["No se han especificado funciones/usuarios específicos de los Tickets"];
            } else {
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))&& !theadminroles.includes(member.id)) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("❌​ No puede eliminar este Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `Todavía no se han configurado los roles de administrador. Hazlo con: \`${prefix}setup-admin\` También puede añadir funciones de sólo Tickets con \`${prefix}setup-ticket\``}`)
                ...
                if (b?.customId == "ticket_verify") {
                    ...
                    await msg.channel.send({
                        embeds: [new Discord.MessageEmbed()
                            ...
                            .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                            .setDescription(`Borrar Ticket en menos de **\`3 Segundos\`** ....\n\n*Si no, puedes hacerlo manualmente*`.substring(0, 2000))
                            .setFooter(client.getFooter(es))
                        ]
                    })
                    setTimeout(() => {
            ...
            create_transcript_buffer([...messageCollection.values()], channel, guild).then(async path => {
                try { // try to send the file
                    ...
                    channel.send({
                        content: `<@${buttonuser.id}>`,
                        embeds: [new MessageEmbed().setAuthor("ERROR! La transcripción es demasiado grande, para ser enviada al Canal!", user.displayAvatarURL({
                            dynamic: true
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("❌​ No puede añadir/eliminar usuarios a/de este Ticket")
                        .setDescription(`${adminroles.length > 0 ? "Necesitas uno de esos Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `Aún no se han configurado las funciones de administración. Hágalo con: \`${prefix}setup-admin\` También puede añadir funciones de sólo Tickets con \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            channel.send({
                ...
                embeds: [new Discord.MessageEmbed()
                    ...
                    .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                    .setDescription(`Ya sea con <@USERID> o con el USERNAME, o con el USERID`.substring(0, 2000))
                    .setFooter(client.getFooter(es))
                ]
            }).then(async msg => {
                            ...
                            if (element.id == user.id) {
                                if (!element.allow.includes("VIEW_CHANNEL")) {
                                    if(!channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.MANAGE_CHANNELS)){
                                        return channel.send(`:x: **Me faltan los permisos MANAGE_CHANNELS para: \`${channel.name}\`**`);
                                    }
                                    ...
                                    if(!channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.MANAGE_CHANNELS)){
                                        return channel.send(`:x: **Me faltan los permisos de MANAGE_CHANNELS para: \`${channel.name}\`**`);
                                    }
                                    channel.permissionOverwrites.edit(user.id, {
                                            SEND_MESSAGES: false,
                                            VIEW_CHANNEL: false,
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("❌​ No puede añadir/eliminar funciones de este Ticket")
                        .setDescription(`${adminroles.length > 0 ? "Necesitas uno de esos Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `Todavía no se han configurado los roles de administrador. Hazlo con: \`${prefix}setup-admin\` También puede añadir funciones de sólo Tickets con \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            channel.send({
                ...
                embeds: [new Discord.MessageEmbed()
                    ...
                    .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                    .setDescription(`Ya sea con <@&ROLEID> o con el ROLEID o con el ROLENAME`.substring(0, 2000))
                    .setFooter(client.getFooter(es))
                ]
            }).then(async msg => {
                msg.channel.awaitMessages({filter: m => m.author.id === buttonuser.id, 
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("❌​ No puede reclamar este Ticket")
                        .setDescription(`${adminroles.length > 0 ? "Necesitas uno de esos Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `Todavía no se han configurado los roles de administrador. Hazlo con: \`${prefix}setup-admin\` También puede añadir funciones de sólo Tickets con \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            ...
            if(!channel.permissionsFor(member).has(Discord.Permissions.FLAGS.SEND_MESSAGES)){
                ...
                channel.permissionOverwrites.edit(member.user, {
                    ...
                    return interaction?.reply({ephemeral: true, content: ":x: **No se pueden cambiar los permisos de usted!**"});
                });
            }
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(!DBindex) {
                ...
                if(interaction.replied) return interaction?.editReply(":x: No se ha podido encontrar la base de datos para su aplicación!");
                else return
            }
            ...
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
               let index = settings.data.findIndex(v => v.value == values[0]);
                if(index < 0) {
                    return interaction?.reply({ephemeral: true, content: ":x: **No se han encontrado los Ticket-Settings para esta opción**"});
                }
                ...
                if (client.setups.get("TICKETS", ticketspath).includes(user.id)) {
                    try {
                      ...
                      if (!ticketchannel || ticketchannel == null || !ticketchannel.id || ticketchannel.id == null) throw {
                        message: "no se ha encontrado ningún canal de tickets, es decir, no hay antispam"
                      }
                      if(client.setups.has(ticketchannel.id) && client.setups.has(ticketchannel.id, "ticketdata"))
                      {
                        ...
                        if(data.state != "closed" && data.menutickettype == 3){
                          return interaction?.reply({content: `❌​ **Ya tiene un Ticket!** <#${ticketchannel.id}>`, ephemeral: true});
                        }
                      }
                    } catch {
                ...
                let optionsData = {
                topic: `📨 Tiecket para: ${user.tag} (${user.id}) | ${values[0]} | ✅ Creado en: ${moment().format("LLLL")}`,
                    type: "GUILD_TEXT",
                    reason: `Sistema de tickets de menú para: ${user.tag}`,
                }
                guild.channels.create(channelname.substring(0, 31), optionsData).then(async ch => {
                await interaction?.reply({content: `🔋 **Creando de su ticket?...** (Suele tardar entre 0 y 2 segundos)`, ephemeral: true});
                try {
                ...
                var ticketembed = new MessageEmbed()
                    .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                    .setFooter(client.getFooter(`To close/manage this ticket react with the buttons\nTambién puede escribir: ${client.settings.get(guild.id, "prefix")}ticket`, es.footericon))
                ...
                if(settings.claim.enabled){
                    ...
                    .setFooter(client.getFooter(es))
                    .setAuthor(client.getAuthor(`Un miembro del personal reclamará el Ticket en breve!`, "https://cdn.discordapp.com/emojis/833101350623117342.gif?size=44", "https://arcticbot.xyz/discord"))
                ...
                let button_close = new MessageButton().setStyle('PRIMARY').setCustomId('ticket_close').setLabel('Cerrar').setEmoji("🔒")
                let button_delete = new MessageButton().setStyle('SECONDARY').setCustomId('ticket_delete').setLabel("Eliminar").setEmoji("🗑️")
                let button_transcript = new MessageButton().setStyle('PRIMARY').setCustomId('ticket_transcript').setLabel("Transcript").setEmoji("📑")
                let button_user = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_user').setLabel("Usuarios").setEmoji("👤")
                ...
                if(settings.claim.enabled){
                    allbuttons.push(new MessageActionRow().addComponents([new MessageButton().setStyle('SECONDARY').setCustomId('ticket_claim').setLabel("Reclamar el Ticket").setEmoji("✅")]))
                }
                ...
                await interaction?.editReply({content: `​✔️ **Creando su Ticket!** <#${ch.id}>`, ephemeral: true});
                })
            }
        }
    })
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(!DBindex) {
                ...
                if(interaction.replied) return interaction?.editReply(":x: No se encuentra la base de datos de su aplicación!");
                else return
            }
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(!DBindex) {
                ...
                if(interaction.replied) return interaction?.editReply(":x: No se encuentra la base de datos para su aplicación!");
                else return
            }
            ...
            if(!data) return interaction?.reply({ephemeral: true, content: ":x: **No se han podido encontrar los datos de este sistema**"});

        ...
        try{
            ...
            if(message.channel.id == chatbot){
              ...
              try{
                ...
                message.channel.send({content: "❌​ LA API DEL CHAT AI NO FUNCIONA"}).catch(() => {})
              }
            }
        ...
        try{
            ...
            for(const user of [...message.mentions.users.values()]){
                if(client.afkDB.has(message.guild.id + user.id)){
                    await message.reply({content: `😭​ **${user.tag}** se fue a AFK <t:${Math.floor(client.afkDB.get(message.guild.id+user.id, "stamp") / 1000)}:R>!${client.afkDB.get(message.guild.id+user.id, "message") && client.afkDB.get(message.guild.id+user.id, "message").length > 1 ? `\n\n__Su mensaje__\n>>> ${String(client.afkDB.get(message.guild.id+user.id, "message")).substring(0, 1800).split(`@`).join(`\`@\``)}` : "" }`}).then(msg=>{
                        setTimeout(()=>{
                            try{
                                msg.delete().catch(() => {});
                            }catch{  }
    ...
    client.on("messageCreate", async message => {
        try{
            ...
            if(message.content && !message.content.toLowerCase().startsWith("[afk]") && client.afkDB.has(message.guild.id + message.author.id)){
                ...
                await message.reply({content: `:tada: Bienvenido de nuevo **${message.author.username}!** :tada:\n> Usted fue <t:${Math.floor(client.afkDB.get(message.guild.id+message.author.id, "stamp") / 1000)}:R> Afk`}).then(msg=>{
                    ...
                    try { 
                        if(!message.deleted) {
                            if(message.channel.permissionsFor(message.channel.guild.me).has(Permissions.FLAGS.MANAGE_MESSAGES)){
                                ...
                                message.reply(":x: **Me falta el permiso MANAGE_MESSAGES!**").then(m => {

    ...
    client.on("messageCreate", async message => {
        ...
        if(set.includes(message.channel.id) || set.includes(message.channel.parentId)){
            try{
                ...
                if (!targetMessage) return console.log("Parece que este mensaje no existe!");
                ...
                if(targetMessage.embeds[0]){
                    ...
                    targetMessage.delete().catch(e=>console.log("ESTE ERROR EVITA UN FALLO"));
                    if(targetMessage.content) return message.channel.send({content: targetMessage.content, embeds: [embed]}).catch(e=>console.log("ESTE ERROR EVITA UN FALLO"));
                    message.channel.send({embeds: [embed]}).catch(e=>console.log("ESTE ERROR EVITA UN FALLO"));
                }
                  //else do this
                else{
                    ...
                    message.channel.send({
                        embeds: [embed],
                        files: files
                    }).catch(e=>console.log("ESTE ERROR EVITA UN FALLO"));
                    message.delete().catch(e=>console.log("ESTE ERROR EVITA UN FALLO"));

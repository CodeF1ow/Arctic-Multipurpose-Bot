...
module.exports = client => {
    ...
    async function welcome(member) {
      if (!client.fetched) {
        if(client.invitations[mem.guild.id]){
          console.log("NO SE HACE CON TODOS LOS SERVIDORES, pero éste sí")
        } else {
          console.log("AÚN NO SE HA CONSEGUIDO, POR FAVOR, ESPERE. Reintento en 5 segundos...");
          setTimeout(() => {
            welcome(member);
          }, 5_000)
          return;
      ...
      if (invite && inviter) {
        ...
        invitemessage = `Invitado por ${inviter.tag ? `**${inviter.tag}**` : `<@${inviter.id}>`}\n👍 **${realinvites} Invitaciones ${realinvites == 1 ? "" : ""}**\n[➡️​ ${invites} Entradas | 🚪 ${leaves} Salidas | ❌​ ${fake} Fakes]`;
      } 
      else if(invite){
        invitemessage = `Invitado por un **Miembro desconocido**`
      }
      else if (vanity) {
        try{
          let res = await member.guild.fetchVanityData().catch(() => {});
          if(res){
            invitemessage = `Invitado por **[Vanity URL](https://discord.gg/${res.code})** con \`${res.uses} Usos\``
          } else {
            invitemessage = `Invitado por **Vanity Link!**`;
          }
        }catch (e){
        ...
        //get the new memberdata
        invitemessage = `Necesito el permiso de **Gestión de Servidor**, para obtener invitaciones!`;
      } else {
        invitemessage = "\u200b"
      }
       ...
       const captcha = new Captcha();
        ...
        if (!mutedrole) {
          mutedrole = await member.guild.roles.create({
            name: `CAPTCHA - Deshabilitado`,
            color: `#222222`,
            hoist: true,
            position: member.guild.me.roles.highest.position - 1,
            reason: `Este rol se creó, a DESHABILITADO - Miembros de CAPTCHA!`
          }).catch((e) => {
            console.log(e.stack ? String(e.stack).grey : String(e).grey);
          });
        }
        ...
        member.send({content: `**${member.guild.name}** tiene habilitada la opción de seguridad Captcha!\n> Resuélvelo primero, escribiendo las LETRAS ENTERAS de la IMAGEN!`, embeds: [captchaembed], files: [attachment]}).then(msg => {
            ...
            if (collected.first().content.trim().toLowerCase() == captcha.text.toLowerCase()) {
              ...
              try{ 
                ...
                if(member.guild.invites.cache.filter(i => i?.code && (i?.maxAge === 0 || i?.maxAge > 600)).size > 0){
                  await member.send(`**OH NO - Has fallado el CAPTCHA!**\n> Aquí hay un enlace de invitación en caso de que lo necesites: https://discord.gg/${member.guild.invites.cache.filter(i => i?.code && i?.maxAge === 0).first().code}\n> :hammer: *Te he expulsado del servidor por motivos de seguridad*`)
                  member.kick("FALLÓ EL CAPTCHA").catch(() => {});
                } else {
                  ...
                  if(channels.size > 0) {
                    member.guild.invites.create(channels.first().id).create().then(async invite => {
                      await member.send(`**OH NO - Has fallado el CAPTCHA!**\n> Aquí hay un enlace de invitación en caso de que lo necesites: https://discord.gg/${invite.code}\n> :hammer: *Te he expulsado del servidor por motivos de seguridad*`)
                      member.kick("FALLÓ EL CAPTCHA").catch(() => {});
                    }).catch(async e => {
                      await member.user.send(`**OH NO - Has fallado el CAPTCHA!**\n> :hammer: *Te he expulsado del servidor por motivos de seguridad*`).catch(() => {})
                      member.kick("FALLÓ EL CAPTCHA").catch(() => {});
                    })
                  }
                }
          ...
          member.guild.channels.create(`verify-${member.user.username}`.substring(0, 32), {
            type: "GUILD_TEXT",
            topic: "POR FAVOR, ENVÍE EL CÓDIGO CAPTCHA EN EL CHAT!",
                ...
                if(ch.permissionsFor(ch.guild.me).has(Discord.Permissions.FLAGS.EMBED_LINKS)){
                  ch.send({
                    ...
                    msg.channel.awaitMessages({filter: m => m.author.id === member.user.id, 
                      ...
                      if (collected.first().content.trim().toLowerCase() == captcha.text.toLowerCase()) {
                        ...
                        try{ 
                          ...
                          //if there is a valid invite which lasts for at least 10 minutes or forever
                          if(member.guild.invites.cache.filter(i => i?.code && (i?.maxAge === 0 || i?.maxAge > 600)).size > 0){
                            await ch.send(`**OH NO - Has fallado el CAPTCHA!**\n> Aquí hay un enlace de invitación en caso de que lo necesites: https://discord.gg/${member.guild.invites.cache.filter(i => i?.code && i?.maxAge === 0).first().code}\n> :hammer: *Te expulsaré en 2 segundos del servidor por razones de seguridad*`)
                            await delay(2000)
                            member.kick("FALLÓ EL CAPTCHA").catch(() => {});
                            ch.delete(() => {});
                          } else {
                            ...
                            if(channels.size > 0) {
                              member.guild.invites.create(channels.first().id).create().then(async invite => {
                                ...
                                await ch.user.send(`**OH NO - Has fallado el CAPTCHA!**\n> :hammer: *Te expulsaré en 2 segundos del servidor por razones de seguridad*`).catch(() => {})
                                await delay(2000)
                                member.kick("FALLÓ EL CAPTCHA").catch(() => {});
                                ch.delete(() => {});
                              })
                    ...
                    member.guild.fetchOwner().then(owner => {
                      owner.send(`:warning: **No puedo crear Channels_with_SEND_MESSAGES_PERMISSIONS para el usuario Captcha, por favor denme PERMISOS para ello lo antes posible!**`).catch(() => {});
                    }).catch(() => {});
                    member.kick().catch(() => {});
                  })
                } else {
            ...
            member.kick().catch(() => {});
            member.guild.fetchOwner().then(owner => {
              owner.send(`:warning: **No puedo crear Canales para el Usuario Captcha, por favor denme PERMISOS para ello lo antes posible!**`).catch(() => {});
            }).catch(() => {});
          })
        })
      } else {
    ...
    if(action == "ban") {
      await member.send({
        embeds: [
          ...
          .setDescription(`Esto se debe a que su cuenta fue creada ${duration(Date.now() - createdAccount).map(a => `\`${a}\``).join(", ")} y el tiempo mínimo de la cuenta debe ser: ${duration(newaccount_delay).map(a => `\`${a}\``).join(", ")}`)
          .addField(`**Guild-Message:**`, `${extramessage && extramessage.length > 1 ? extramessage : "No Extra Message provided"}`.substring(0, 1024))
        ]
      }).catch(() => {});
      member.ban({reason: `Detección de cuentas alternativas | Cuenta creada ${duration(Date.now() - createdAccount).join(", ")} hace`})
    } else {
      await member.send({
        embeds: [
          new Discord.MessageEmbed()
          ...
          .setThumbnail(member.guild.iconURL({dynamic: true}))
          .setFooter({text: `${member.guild.name} | ${member.guild.id}`, iconURL: `${member.guild.iconURL({dynamic: true})}`})
          .setDescription(`Esto es porque su cuenta fue creada ${duration(Date.now() - createdAccount).map(a => `\`${a}\``).join(", ")} y el tiempo mínimo de la cuenta debe ser: ${duration(newaccount_delay).map(a => `\`${a}\``).join(", ")}`)
          .addField(`**Guild-Message:**`, `${extramessage && extramessage.length > 1 ? extramessage : "No Extra Message provided"}`.substring(0, 1024))
        ]
      }).catch(() => {});
      member.kick({reason: `Detección de cuentas alternativas | Cuenta creada ${duration(Date.now() - createdAccount).join(", ")} hace`})
    }
  })
  ...
  client.on("guildMemberAdd", async member => {
    ...
    if(!member.user.avatarURL() && joinlist.noavatar.filter(d => d.data == "enable").length > 0) {
      const reason = '`Usuario sin avatar (joinlist)`'
    ...
    if(joinlist.username_contain.map(d => d.data).some(d => member.user.username.toLowerCase().includes(d.toLowerCase()))) {
      const reason = '`El nombre de usuario contiene ${data.data} (joinlist)`'
    ...
    if(joinlist.username_equal.map(d => d.data).some(d => d.toLowerCase() == member.user.username.toLowerCase())) {
      const reason = '`El nombre de usuario es igual a ${data.data} (joinlist)`'
    ...
    if(joinlist.userid.map(d => d.data).some(d => d == member.id)) {
      const reason = '`El ID de usuario es igual a ${data.data} (joinlist)`'
    ...
    if(joinlist.server_in_common.map(d => d.data).length > 0) {
      ...
      const reason = '`Usted está en el servidor ${inthere.name} (joinlist)`'
    ...
    if(joinlist.server_not_in_common.map(d => d.data).length > 0) {
      ...
      const reason = '`No estás en el servidor ${notInthere.name} (joinlist)`'
      const datas = joinlist.server_not_in_common.filter(d => d.data == notInthere.id);
      await handleDatas(datas, reason);
    }

    function handleDatas(datas, reason = "No se ha proporcionado ninguna razón") {
      return new Promise(async (resolve, reject) => {
        if(datas.length > 0) {
          for(const data of datas) {
            if(data.action == "kick") {
              if(member.kickable) {
                await member.send(`Te han expulsado de \`${member.guild.name}\` porque:\n> ${eval(reason)}`).catch(() => {});
                await member.kick({reason: `${eval(reason)}`}).catch(console.warn)
              }
            } 
            if(data.action == "ban") {
              if(member.bannable) {
                await member.send(`Te han expulsado de \`${member.guild.name}\` for ${data.days != 0 ? `${data.days} Días` : `ever`} porque:\n> ${eval(reason)}`).catch(() => {});
                await member.ban({reason: `${eval(reason)}`, days: data.days }).catch(console.warn)
              }
            } 
            if(data.time && data.time > 0 && data.action == "timeout") {
              if(member.manageable) {
                await member.send(`Tienes tiempo de espera hasta que <t:${Math.floor((Date.now() + data.time) / 1000)}:F> de \`${member.guild.name}\` porque:\n> ${eval(reason)}`).catch(() => {});

...
const Buttons = {
  acceptbutton: new MessageButton().setStyle('SUCCESS').setEmoji(`✅`).setCustomId(`Apply_accept`).setLabel(`Aceptar`),
  declinebutton: new MessageButton().setStyle('DANGER').setEmoji(`❌`).setCustomId(`Apply_deny`).setLabel(`Rechazar`),
  ticketbutton: new MessageButton().setStyle('SECONDARY').setEmoji(`🎟️`).setCustomId(`Apply_ticket`).setLabel(`Preguntar`),
  ...
  acceptbutton_d: new MessageButton().setStyle('SUCCESS').setEmoji(`✅`).setCustomId(`Apply_accept`).setLabel(`Aceptar`).setDisabled(true),
  declinebutton_d: new MessageButton().setStyle('DANGER').setEmoji(`❌`).setCustomId(`Apply_deny`).setLabel(`Rechazar`).setDisabled(true),
  ticketbutton_d: new MessageButton().setStyle('SECONDARY').setEmoji(`🎟️`).setCustomId(`Apply_ticket`).setLabel(`Preguntar`).setDisabled(true),
...
module.exports = client => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    if(!index) {
      if(!interaction.replied) return interaction?.reply({ephemeral: true, content: `:x: No se ha podido encontrar la base de datos para su aplicación!`});
      else return interaction?.editReply({ephemeral: true, content: `:x: No se pudo encontrar la base de datos para su aplicación!`});
    }
    ...
    try {
      ...
      interaction?.reply({content: `Iniciar la aplicación en sus **mensajes directos!**`, ephemeral: true}).catch(() => {});
      ...
      function ask_question(qu) {
        ...
        user.send({embeds: [new Discord.MessageEmbed()
            ...
            .setDescription(qu)
            .setAuthor(client.getAuthor(`Pregunta ${counter + 1} / ${Questions.length}`, client.user.displayAvatarURL(), `https://discord.gg/MBPsvcphGf`))
            .setFooter(client.getFooter(es))
        ]}).then(msg => {
            msg.channel.awaitMessages({ filter: m => m, 
              ...
              if (collected.first().attachments.size > 0) {
                if (collected.first().attachments.every(attachIsImage)) {
                  ...
                  answers.push(`${collected.first().content}\nHabía un archivo adjunto, que no puedo mostrar!`);
                }
              } else {
                answers.push(`${collected.first().content}`);
              }
      ...
      async function send_finished() {
        if (apply_db.get(guild.id, `${pre}.last_verify`)) {
              ...
              .then(async collected => {
                ...
                if (reaction.emoji?.name === `✅`) {
                  ...
                  const finished_embed = new Discord.MessageEmbed()
                    ...
                    .addField(`\u200b`, `**❯** Volver al canal ${originalchannel}`).setFooter(client.getFooter(es))
                  ...
                  try {
                    ...
                    if (roleid) {
                      if (roleid.length == 18) {
                        ...
                        //add the role
                        member.roles.add(role.id).catch((e)=>{channel_tosend.send(`Me faltan permisos para conceder el rol\n` + e.message)});
                      }
                    }

                  } catch (e) {
                    console.log(e.stack ? String(e.stack).grey : String(e).grey)
                    channel_tosend.send(`Me faltan permisos para conceder el SOLICITADO\n` + e.message)
                    /* */
                  }
          ...
          const finished_embed = new Discord.MessageEmbed()
            ...
            .addField(`\u200b`, `**❯** Volver al canal ${originalchannel}`).setFooter(client.getFooter(es))
          originaluser.send({content: `**❯** Volver al canal ${originalchannel}`, embeds: [finished_embed]})
          ...
          try {
            ...
            if (roleid) {
              if (roleid.length == 18) {
                ...
                //add the role
                member.roles.add(role.id).catch((e)=>{channel_tosend.send(`Me faltan permisos para dar el rol\n` + e.message)});
              }
            }
  ...
  client.on(`interactionCreate`, async (interaction) => {
    ...
    if (interaction?.customId.startsWith(`Apply_`)) {
        ...
        if(!index) {
          return console.log(`NO SE ENCUENTRA DB`)
        }
        let applyname = `apply${index}`;
        let applytickets = `applytickets${index}`; 
        let ticketsetupapply = `ticket-setup-apply-${index}`; 
        ...
        try {
          ...
          const embed = new Discord.MessageEmbed()
            ...
            .setDescription(`${oldEmbed.description ? `${oldEmbed.description}\n`: ``} Editado por: <@${interaction?.user.id}> | ${emoji[interaction?.customId]}`.substr(0, 2048))
          ...
          if (interaction?.customId == `Apply_accept`) {
            ...
            try {
              ...
              if (roleid) {
                ...
                member.roles.add(roleid).catch((e)=>{channel_tosend.send(`Me faltan permisos para dar el rol\n` + e.message)});
              }
            } catch (e) {
          ...
          if (interaction?.customId === `Apply_ticket`) {
            ...
            try {
              message.guild.channels.create(`Ticket-${usert.username}`.substr(0, 32), {
                  type: 'text',
                  topic: `Simplemente borre este canal, si no lo necesita no hay comando de borrado/cierre!`,
                ...
                .then((channel) => {
                  ...
                  let button_delete = new MessageButton().setStyle('SECONDARY').setCustomId('ticket_delete').setLabel(`Borrar`).setEmoji(`🗑️`)
                  let button_transcript = new MessageButton().setStyle('PRIMARY').setCustomId('ticket_transcript').setLabel(`Transcript`).setEmoji(`📑`)
                  let button_user = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_user').setLabel(`Gestionar Usuarios`).setEmoji(`👤`)
                  let button_role = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_role').setLabel(`Gestionar Roles`).setEmoji(`📌`) 
                  ...
                  try {
                    if(client.setups.get(`TICKETS`, applytickets).includes(usert.id)){
                      channel.send({ 
                        content: `<@${usert.id}>\nDebido a que ya tiene un TICKET para este Sistema de Aplicación, se creó este Canal!`,
                        embeds: [new Discord.MessageEmbed()
                              .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes(`http://`) || es.footericon.includes(`https://`)) ? es.footericon : client.user.displayAvatarURL() : null)
                              .setTitle(eval(client.la[ls][`handlers`][`applyjs`][`apply`][`variable36`]))
                              .setFooter(client.getFooter(`Simplemente borre este canal, si no lo necesita no hay comando de borrado/cierre!`, message.guild.iconURL({
                                dynamic: true
                              })))
                      ...
                      channel.send({ 
                        ...
                        embeds: [new Discord.MessageEmbed()
                              ...
                              .setFooter(client.getFooter(`Para cerrar/gestionar este ticket reaccione con los botones
You can also type: ${client.settings.get(channel.guild.id, `prefix`)}ticket!`, message.guild.iconURL({
                  ...
                  try {
                    ...
                    var approve = new Discord.MessageEmbed()
                      ...
                      .setFooter(client.getFooter(`Por: ` + interaction?.user.tag, interaction?.user.displayAvatarURL({
                        dynamic: true
                      })))
                      .setDescription(apply_db.get(message.guild.id, `${pre}.ticket`).replace(`{user}`, `<@${usert.id}>`) + ` Canal: <#${channel.id}>`)

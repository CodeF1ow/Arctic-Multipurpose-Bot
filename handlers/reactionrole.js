 ...
 **** EXAMPLE DB STRUCTURE:
    ...
    const ReactionRoleData = {
        ...
        Parameters: [ // Technically you could add infnite parameters (emojis), but discord just allows 20 at once on 1 message ...
            ...
            { Emoji: '866089513654419466', Emojimsg: '<:Builder:1015898430469587004>', Role: '845952370889981952' }, // Discord Custom Emoji
                            ...
                            try {
                                ...
                                if (guildRole) {
                                    if (messagereaction.guild.me.roles.highest.rawPosition > guildRole.rawPosition) {
                                        ...
                                        reaction.message.channel.send("El Rol está por encima de mi Rol más alto, no puedo dártelo!").then(msg => {
                                            setTimeout(() => msg.delete().catch(() => {}), 3000)
                                        }).catch(() => {})
                                    }
                                } else {
                                    reaction.message.channel.send("Este rol fue borrado, no puedo dártelo!").then(msg => {
                                        setTimeout(() => msg.delete().catch(() => {}), 3000)
                                    }).catch(() => {})
                                }
                            } catch (error) {
                                ...
                                if (rr2[z].Role != currrole) {
                                    ...
                                    if (guildRole) {
                                        if (messagereaction.guild.me.roles.highest.rawPosition > guildRole.rawPosition) {
                                            ...
                                            reaction.message.channel.send("El rol está por encima de mi rol más alto, no puedo quitártelo!").then(msg => {
                                                setTimeout(() => msg.delete().catch(() => {}), 3000)
                                            }).catch(() => {})
                                        }
                                    } else {
                                        reaction.message.channel.send("Este rol fue borrado, no puedo eliminarlo para ti!").then(msg => {
                                            setTimeout(() => msg.delete().catch(() => {}), 3000)
                                        }).catch(() => {})
                                    }
                                }

...
const dirSetup = [
    {
            "Folder": "Info",
            "CmdName": "info",
            "CmdDescription": "Obtener información!"
    },
    {
            "Folder": "Music",
            "CmdName": "music",
            "CmdDescription": "Escuchar música!"
    },
    {
            "Folder": "Admin",
            "CmdName": "admin",
            "CmdDescription": "Administrar el servidor!"
    },
    {
            "Folder": "Fun",
            "CmdName": "fun",
            "CmdDescription": "Comandos relacionados con la diversión!"
    }
];
				...
				if(cmdSetup && cmdSetup.Folder) {
					...
					for (let file of slashCommands) {
						...
						if (pull.name && pull.description) {
								...
								if(pull.options && pull.options.length > 0){
									for(const option of pull.options){
										if(option.User && option.User.name && option.User.description){
											...
											console.log(`A la Opción le falta el Nombre o/y la Descripción de ${pull.name}`)
										}
									}
								}
								return subcommand;
							...
							console.log(file, `error -> falta un help.name, o help.name no es una cadena.`.brightRed);
							continue;
						}
					}
					...
					return console.log(`La carpeta de subcomandos ${dir} no está en la configuración de dirSetup!`);
				}
			} else {

        ...
        if(Queuechannel && Queuechannel.permissionsFor(Queuechannel.guild.me).has(Permissions.FLAGS.SEND_MESSAGES)){
          Queuechannel.messages.fetch(player.get("currentmsg")).then(currentSongPlayMsg => {
            if(currentSongPlayMsg && currentSongPlayMsg.embeds && currentSongPlayMsg.embeds[0]){
              ...
              embed.footer.text += "\n\n⛔️ CANCIÓN Y COLA TERMINADA! | El jugador fue EXPULSADO (detenido)"
    ...
    .on("trackStart", async (player, track) => {
      try {
        ...
        if(playercreated.has(player.guild)){
          player.set("eq", "💣 Ninguno");
          player.set("filter", "🧨 Ninguno");
        ...
        if(channel && channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.SEND_MESSAGES)){
          ...
          collector.on('collect', async i => {
              ...
              if (!player)
                return i?.reply({content: "❌ Todavía no hay nada en juego", ephemeral: true})
              ...
              if (channel.id !== player.voiceChannel)
                return i?.reply({
                  content: `❌ **Por favor, únase a __my__ El canal de voz es lo primero. <#${player.voiceChannel}>**`,
                  ephemeral: true
                })
              ...
              if(i?.customId == "1") {
                ...
                if (player.queue.size == 0) {
                  ...
                  i?.reply({
                    ...
                    .setColor(ee.color)
                    .setTimestamp()
                    .setTitle(`⏹ **Dejó de sonar y abandonó el Canal**`)
              ...
              if(i?.customId == "4") {
                ...
                i?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `​✔️ **Activar Reproducción automática**`: `❌ **Desactivación de la reproducción automática**`}`)
                  .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
              }
              ...
              if(i?.customId == `5`){
                ...
                i?.reply({
                  embeds: [new MessageEmbed()
                    .setColor(ee.color)
                    .setTimestamp()
                    .setTitle(`🔀 **Barajar ${player.queue.length} Canciones!**`)
                    .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
              }
              ...
              if(i?.customId == `6`){
                ...
                i?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `​✔️ **Bucle de canción activado**`: `❌ **Bucle de canciones desactivado**`}`)
                  .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                var data = generateQueueEmbed(client, player, track)
                swapmsg.edit(data).catch((e) => {
              ...
              if(i?.customId == `7`){
                ...
                i?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `​✔️ **Bucle de cola activado**`: `❌ **Bucle de cola desactivado**`}`)
                  .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                var data = generateQueueEmbed(client, player, track)
                swapmsg.edit(data).catch((e) => {
              ...
              if(i?.customId == `8`){
                ...
                i?.reply({
                  embeds: [new MessageEmbed()
                    .setColor(ee.color)
                    .setTimestamp()
                    .setTitle(`⏩ **Reenvió la canción para \`10 segundos\`!**`)
                    .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
              }
              ...
              if(i?.customId == `9`){
                ...
                i?.reply({
                  embeds: [new MessageEmbed()
                    .setColor(ee.color)
                    .setTimestamp()
                    .setTitle(`⏪ **Rebobina la canción para \`10 Segundos\`!**`)
                    .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
              }
          });  
...
var embed = new MessageEmbed().setColor(ee.color)
  ...
  embed.setFooter(client.getFooter(`Solicitado por: ${track.requester.tag}`, track.requester.displayAvatarURL({dynamic: true})));
let skip = new MessageButton().setStyle('PRIMARY').setCustomId('1').setEmoji(`⏭`).setLabel(`Saltar`)
...
let shuffle = new MessageButton().setStyle('PRIMARY').setCustomId('5').setEmoji('🔀').setLabel(`Barajar`)
if (!player.playing) {
...
let songloop = new MessageButton().setStyle('SUCCESS').setCustomId('6').setEmoji(`🔁`).setLabel(`Canción`)
let queueloop = new MessageButton().setStyle('SUCCESS').setCustomId('7').setEmoji(`🔂`).setLabel(`Cola`)
let forward = new MessageButton().setStyle('PRIMARY').setCustomId('8').setEmoji('⏩').setLabel(`+10 Seg`)
let rewind = new MessageButton().setStyle('PRIMARY').setCustomId('9').setEmoji('⏪').setLabel(`-10 Seg`)
let lyrics = new MessageButton().setStyle('PRIMARY').setCustomId('10').setEmoji('📝').setLabel(`Letras`).setDisabled();

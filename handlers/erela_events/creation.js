...
var {
  ...
  function collect(node) {
    return node.map(x => {
      ...
      if (!x.host) throw new RangeError('"host" debe proporcionarse');
      if (!x.password) throw new RangeError('"password" debe proporcionarse');
      if (typeof x.port !== 'number') throw new RangeError('"port" debe ser un número');
      if (x.retryAmount && typeof x.retryAmount !== 'number') throw new RangeError('Retry amount debe ser un número');
      if (x.retryDelay && typeof x.retryDelay !== 'number') throw new RangeError('Retry delay debe ser un número');
      if (x.secure && typeof x.secure !== 'boolean') throw new RangeError('Secure debe ser un booleano');

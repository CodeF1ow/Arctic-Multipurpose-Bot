...
module.exports = {
  ...
  cooldown: 5,
  options: [
		{"String": { name: "chat_text", description: "Quieres chatear conmigo??", required: false }}, 
  ],
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    ...
    //console.log(interaction, StringOption)
     ...
     .then(data => {
       if(!data.cnt){
        interaction?.editReply({content: ":cry: **Lo siento, no tengo ni idea... No puedo conectarme a la API!**", ephemeral: true}).catch(e => console.log("CHATBOT:".underline.red + " :: " + e.stack.toString().grey));
       }else{

...
const { handlemsg } = require(`${process.cwd()}/handlers/functions`);
    module.exports = {
  name: `join`,
  description: `Invoca al Bot en su Canal`,
  parameters: {
    "type": "radio",
    "activeplayer": false,
    "previoussong": false
  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    ...
    try {
      ...
      if(!interaction.member?.voice.channel?.permissionsFor(message.guild?.me)?.has(Permissions.FLAGS.CONNECT)) 
        return message.reply({ content: "​❌​ **Me falta el permiso para conectarme a su canal de voz!**"}).catch(() => null);
      if(!interaction.member?.voice.channel?.permissionsFor(message.guild?.me)?.has(Permissions.FLAGS.SPEAK)) 
        return message.reply({ content: "​❌​ **Me falta el permiso para hablar en su canal de voz!**"}).catch(() => null);

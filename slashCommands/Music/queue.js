...
module.exports = {
  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    ...
    try {
      ...
      if (!tracks.length)
        return interaction?.reply({ephemeral: true, embeds : [new MessageEmbed()
          .setAuthor(`Cola de ${message.guild.name}  -  [ ${tracks.length} Tracks ]`, message.guild.iconURL({
            dynamic: true
          }))
          .setColor(es.color)
      ...
      if (tracks.length < 15)
        return interaction?.reply({ephemeral: true, embeds :[new MessageEmbed()
          .setAuthor(`Cola de ${message.guild.name}  -  [ ${player.queue.length} Tracks ]`, message.guild.iconURL({
            dynamic: true
          }))
          .addField(eval(client.la[ls]["cmds"]["music"]["queue"]["variablex_3"]), eval(client.la[ls]["cmds"]["music"]["queue"]["variable3"]))
          .setColor(es.color).setDescription(tracks.map((track, i) => `**${++i})** **${track.title.substring(0, 60)}** - \`${track.isStream ? `TRANSMISIÓN EN VIVO` : format(track.duration).split(` | `)[0]}\`\n*requested by: ${track.requester.tag}*`).join(`\n`))
        ]}).then(msg => {
          setTimeout(()=>{
            try { 
              msg.delete().catch(() => null);
      ...
      for (let i = 0; i < tracks.length; i += 15) {
        ...
        quelist.push(songs.map((track, index) => `**${i + ++index})** **${track.title.substring(0, 60)}** - \`${track.isStream ? `TRANSMISIÓN EN VIVO` : format(track.duration).split(` | `)[0]}\`\n*requested by: ${track.requester.tag}*`).join(`\n`))
      }
      ...
      for (let i = 0; i < limit; i++) {
        ...
        await embeds.push(new MessageEmbed()
          .setAuthor(`Cola de ${guild.name}  -  [ ${player.queue.length} Pistas ]`, guild.iconURL({
            dynamic: true
          }))
          .setColor(es.color)

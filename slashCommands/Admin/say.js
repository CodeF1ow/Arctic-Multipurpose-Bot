...
module.exports = {
  ...
  description: "Enviar un texto al chat", //the command description for Slash Command Overview
  cooldown: 5,
  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    try{
		...
		await interaction?.reply({content: `Enviar el mensaje...`, ephemeral: true}).catch(() => null);
		//SEND THE EMBED!
		await channel.send({content: String(Text).substring(0, 2000).split("+n+").join("\n")}).catch(() => null);
		//Edit the reply
		interaction?.editReply({content: `✅ Mensaje enviado en ${channel}!`, ephemeral: true}).catch(() => null);
    } catch (e) {

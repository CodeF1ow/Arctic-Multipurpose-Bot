...
module.exports = {
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
	//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		...
		{"String": { name: "title", description: "Cuál debería ser el título de Embed?", required: true }}, //to use in the code: interacton.getString("title")
		{"String": { name: "description", description: "Cuál debería ser la descripción de Embed? [ +n+ = NewLine ]", required: true }}, //to use in the code: interacton.getString("description")
		{"String": { name: "color", description: "Cuál debe ser el color Embed?", required: false }}, //to use in the code: interacton.getString("color")
		//{"User": { name: "ping_a_user", description: "To Ping a user lol", required: false }}, //to use in the code: interacton.getUser("ping_a_user")
		{"Channel": { name: "in_where", description: "En qué canal debo enviarlo?", required: false }}, //to use in the code: interacton.getChannel("what_channel")
  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    try{
		...
		await interaction?.reply({content: `Envío del embed...`, ephemeral: true}).catch(() => null);
		//SEND THE EMBED!
		await channel.send({embeds: [embed]}).catch(()=>{
			channel.send({embeds: [embed.setColor(es.color)]}).catch(() => null);
		})
		//Edit the reply
		interaction?.editReply({content: `✅ Embed enviados ${channel}!`, ephemeral: true}).catch(() => null);
    } catch (e) {

...
module.exports = {
  name: "ping",
  description: "Te da información sobre la rapidez con la que el Bot puede responderte",
    ...
    try {
      interaction?.reply({ephemeral: true, embeds: [new MessageEmbed()
        ...
        interaction?.editReply({ephemeral: true, embeds: [new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          .setFooter(client.getFooter("Se tarda más, porque estoy recibiendo mi ping de acogida!", es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL()))

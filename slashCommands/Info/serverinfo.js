  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    ...
    try {
      function trimArray(arr, maxLen = 25) {
        if (arr.map(this_Code_is_by_Kiri86_8565 => this_Code_is_by_Kiri86_8565).length > maxLen) {
          const len = arr.map(this_Code_is_by_Kiri86_8565 => this_Code_is_by_Kiri86_8565).length - maxLen;
          arr = arr.map(this_Code_is_by_Kiri86_8565 => this_Code_is_by_Kiri86_8565).sort((a, b) => b?.rawPosition - a.rawPosition).slice(0, maxLen);
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .addField(client.la[ls].cmds.info.serverinfo.field12, "<:nitro_logo:966470998557880351> \`" + message.guild.premiumSubscriptionCount + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field13, "<:nitro_logo:966470998557880351> \`" + boostlevel + "\`", true)

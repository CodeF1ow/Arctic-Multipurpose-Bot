  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message, GuildSettings) => {
    ...
    try{
      ...
      interaction?.reply({ephemeral: true, embeds: [new Discord.MessageEmbed()
        ...
        .addField(client.la[ls].cmds.info.invites.field3.title, `>>> \`\`\`yml\nEntradas - Fakes - Salidas = Invitaciones reales\n${invites}${" ".repeat("Entradas ".length -String(invites).length)}- ${fake}${" ".repeat("Fakes ".length -String(fake).length)}- ${leaves}${" ".repeat("Salidas ".length -String(leaves).length)}= ${realinvites}\n\`\`\``)
